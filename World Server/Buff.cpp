    ...
    switch( thisskill->buff[i])
    {
        case 16: // HP
        {
            if(thisskill->duration <= 0 )//is heal
            {
                ...
                int heal = 80 + thisskill->value1[i] + character->GetInt() * 9;
                Log(MSG_INFO,"Heal value is %i", heal);
                ...
                //Log(MSG_INFO,"Heal amount: %i Character %i HP is now %I64i", heal, character->clientid, character->Stats->HP);
                return true;
            }
        }
        break;
        ...
        break;
        case 38: // Terr0risT fix Bonfire HP heal / Salamander HP heal
        {
            if(thisskill->duration <= 0 )//is heal
            {
                if(character->Stats->HP <= 0) //character is dead
                ...
                int hpvalue = thisskill->value2[i];
                int hpheal = character->Stats->MaxHP * hpvalue / 100;
				character->Stats->HP += hpheal;

                if(character->Stats->HP > character->Stats->MaxHP )
                {
                    character->Stats->HP = character->Stats->MaxHP;
                }
                //Log(MSG_INFO,"HP Heal: %i Character HP is now %I64i", hpheal, character->Stats->HP);
                return true;
            }
        }
        break;
        case 39: // Terr0risT fix Bonfire MP heal / Mana Flames MP heal
        {
            if(thisskill->duration <= 0 )// recover MP
            {
                int mpvalue = thisskill->value2[i];
                int mpheal = character->Stats->MaxMP * mpvalue / 100;
                character->Stats->MP += mpheal;

                if(character->Stats->MP > character->Stats->MaxMP)
                {
                    character->Stats->MP = character->Stats->MaxMP;
                }
                //Log(MSG_INFO,"MP Heal: %i Current MP is now %I64i", mpheal, character->Stats->MP);
                return true;
            }
        }
        break;
        ...
        switch( thisskill->status[i])
        {
            ...
            if(randv <= thisskill->success)
            {
                ...
                if(BuffValue.Position != 0xff)
                {
                    ...
                    if(j==BUFF_POISONED_POS)
                    {
                        ...
                        //character->MagicStatus[j].Buff = thisskill->atkpower;
                        character->MagicStatus[j].BuffTime = clock();
                        character->MagicStatus[j].Duration = thisskill->duration;
                        character->MagicStatus[j].Value = 0; // Original value is 0
            ...
            case STATUS_ENCHANT_DAMAGE2: //Valkyrie Charm:
            {
                ...
                if(BuffValue.NewValue!=0)
                {
                    ...
                    character->Stats->ExtraDamage_add = thisskill->atkpower; // Valkyrie Damage FIX - Terr0risT
...
//                    character->Stats->ShieldDamage = thisskill->atkpower;
            ...
            if(randv <= thisskill->success)
            {
                ...
                if(BuffValue.NewValue!=0)
                {
                    ...
                    Log( MSG_INFO, "Defence buff position %i. Deff(%i->%i). buff value %i", j,prev_value,BuffValue.NewValue,BuffValue.Value );
                    character->MagicStatus[j].Buff = thisskill->buff[i];
                    character->MagicStatus[j].BuffTime = clock();
                    character->MagicStatus[j].Duration = thisskill->duration;
                    character->MagicStatus[j].Value = BuffValue.Value;
            ...
            if(randv <= thisskill->success)
            {
                ...
                if(BuffValue.NewValue!=0)
                {
                    ...
                    character->Stats->Attack_Speed_Percent *= BuffValue.Value/100;
                    ...
                    Log( MSG_INFO, "New Attack Speed buff position %i. ASpeed(%i -> %i). buff value %i", j,prev_value,BuffValue.NewValue,BuffValue.Value );
                    character->MagicStatus[j].Buff = thisskill->buff[i];
                    character->MagicStatus[j].BuffTime = clock();
                    character->MagicStatus[j].Duration = thisskill->duration;
                    character->MagicStatus[j].Value = BuffValue.Value;
            ...
            if(randv <= 100) // 100% for now..
            {
                ...
                if(BuffValue.Position != 0xff)
                {
                        ...
                        character->MagicStatus[j].Buff = thisskill->status[i]; // Original Value
                        character->MagicStatus[j].BuffTime = clock();
                        character->MagicStatus[j].Duration = thisskill->duration;
                        character->MagicStatus[j].Value = 0; // this might cause headaches later
                        character->MagicStatus[j].Status = thisskill->status[i];
                        Log( MSG_INFO, "Detect Status %i applied for %i to %i",character->MagicStatus[j].Status, thisskill->duration, character->clientid);
                        bflag = true;
                }
            }
            break;
...
CBValue CWorldServer::GetBuffValue( CSkills* thisskill, CCharacter* character, int intvalue, UINT i, UINT up, UINT down, UINT CurrentValue, bool Buff, bool Status )
{
    ...
    switch(thisskill->status[i])
    {
        ...
        /* - Not coded for now.. Terr0risT
        case STATUS_REVIVE:
            NewValue.Position = BUFF_REVIVE_POS;
            break;

        case STATUS_DETECT:
            NewValue.Position = BUFF_DETECT_POS;
            break;
        */
        ...
        case STATUS_BURN_30: case STATUS_BURN_60: case STATUS_BURN_100: case STATUS_BURN_150: case STATUS_BURN_200: case STATUS_BURN_300: case STATUS_BURN_400:
            NewValue.Position = BUFF_BURN_POS;
            break;
    ...
    {
        Log(MSG_DEBUG,"No position for %i",thisskill->status[i]);
        return NewValue;
    }
