...
PVOID MapProcess( PVOID TS )
{
    ...
    UINT delay_ping3=2000;
    UINT last_group_update=clock();
    ...
    while(GServer->ServerOnline)
    {
        ...
        UINT etime10 = (UINT)round((clock( ) - last_group_update));
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            if (!only_npc||only_summon)
            {
                ...
                for(UINT j=0;j<map->PlayerList.size();j++)
                {
                    ...
                    if(etime10>=delay_ping3) //2 seconds
                    {
                        GServer->RefreshGroups(player);
                        last_group_update=clock();
                    }
                ...
                for(UINT j=0;j<map->MonsterList.size();j++)
                {
                   ...
                   //PY utterly pointless. UpdateValues always returns true
                    ...
                    if(monster->IsOnBattle( )&& !monster->IsDead())
                    {
                        //monster->DoAttack( );
                        //LMA: AIP Timer.
