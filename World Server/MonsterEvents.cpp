...
{
    ClearBattle(Battle);
    Log(MSG_INFO,"An enemy died, BATTLE was cleared."); //Log turned off.
...
bool CMonster::SummonUpdate(CMonster* monster, CMap* map, UINT j)
{
    ...
    if( etime >= 4 * CLOCKS_PER_SEC && Stats->HP > 0 )
    {
        ...
        {
            die_amount=(UINT)(monster->Stats->MaxHP/monster->life_time); // Commented out - Terr0risT
            //die_amount=1; // Terr0risT wants summons to last longer.
        ...
        Stats->HP -= (die_amount); // For now, only die_amount no x4 - Terr0risT  //*4 because each 4 seconds ;)
        lastDegenTime = clock();
        if (Stats->HP < 1||ownerclient == NULL||ownerclient->IsDead())//orphan check
        {
            ...
            if(ownerclient != NULL&&monster->owner_user_id>0)
            {
                ...
                Log(MSG_INFO,"Orphaned summon died (took %u), jauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,ownerclient->cur_jauge,ownerclient->summon_jauge);
            }
            else
            {
                ...
                if(monster->owner_user_id>0)
                {
                    ...
                    if(tempplayer!=NULL)
                    {
                        ...
                        Log(MSG_INFO,"summon died because Owner Warped (took %u), jauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,tempplayer->cur_jauge,tempplayer->summon_jauge);
                    }
    ...
    else if ( Stats->HP < 1 )
    {
        ...
        if(ownerclient != NULL&&monster->owner_user_id>0)
        {
            ...
            Log(MSG_INFO,"The summon DIED (took %u), jauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,ownerclient->cur_jauge,ownerclient->summon_jauge);
        }
        else
        {
            //LMA: We have to look for the owner with his userid, perhaps he warped...
