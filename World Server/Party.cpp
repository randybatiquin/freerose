    ...
    switch(action)
    {
        ...
        case 0x01://invite a existent party
        {
            ...
            if(otherclient==NULL)
            {
                ...
                ADDBYTE    ( pak, 0x00 ); // Cannot find ID
            ...
            {
                BEGINPACKET( pak, 0x7d1 );
                ADDBYTE    ( pak, 0x01 );// Cannot be added / Already in a party
                ADDWORD    ( pak, clientid );
                ADDBYTE    ( pak, 0x00 );
                thisclient->client->SendPacket( &pak );
                return true;
            ...
            {
                BEGINPACKET( pak, 0x7d1 );
                ADDBYTE    ( pak, 0x07 );// Inappropriate level
                ADDWORD    ( pak, clientid );
                ADDBYTE    ( pak, 0x00 );
                thisclient->client->SendPacket( &pak );
                return true;
...
bool CWorldServer::pakPartyManager( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        case 0x02://Acepto
        {
            ...
            thisclient->client->SendPacket( &pak );
            thisclient->Party->IsMaster = false;
            // Observe: Send New Party Member info to other players
            RESETPACKET( pak, 0x7d2 );
            ...
            // Send the New Party Member the members List
            RESETPACKET( pak, 0x7d2 );
            ADDBYTE    ( pak, party->Option );
...
bool CWorldServer::pakPartyChat( CPlayer* thisclient, CPacket* P )
{
    ...
    Log(MSG_PARTY,"%s: %s", thisclient->CharInfo->charname, P->Buffer );
    return true;
}
