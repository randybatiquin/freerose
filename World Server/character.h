...
class CCharacter
{
    public:
        ...
        BATTLE* Battle;
        STATUS* Status;
        MAGICSTATUS MagicStatus[64];
        MAGICSTATUS2 MagicStatus2[64];
        int shield_reflect;
       ...
       int ExecuteQuestTrigger(dword hash,UINT index=0);
		...
		bool ResumeNormalAttack( CCharacter* Enemy,bool was_aoe=false);    //LMA: Do we have to resume NormalAttack? - // Added again by Terr0risT
        ...
        void UseAtkSkill( CCharacter* Enemy, CSkills* skill, bool deBuff= false, bool aoe= false );
        ...
        virtual unsigned int GetStr( );
        virtual unsigned int GetDex( );
        virtual unsigned int GetInt( );
        virtual unsigned int GetCha( );
        virtual unsigned int GetSen( );
        virtual unsigned int GetCon( );
        virtual unsigned int GetClassRequiredStatus( );
