...
void CCharacter::StartAction( CCharacter* Target, BYTE action, UINT skillid, bool restart, CCharacter* receiver )
{
    ...
    // - No need to log this for now.. - Terr0risT
    //else
    //{
    ...
    switch(action)
    {
        ...
        case AOE_TARGET:
        {
            ...
            // end Terr0risT Fix for AOE skills.
            ...
            Position->lastMoveTime = clock( );
            Log(MSG_INFO,"Start AOE Action, AOE_TARGET, target (%.2f,%.2f)",Position->aoedestiny.x,Position->aoedestiny.y);
        }
        break;
...
{
    // if (Status->Sleep != 0xff || Status->Stun != 0xff)return false;  // Temporarily turned OFF
    clock_t etime = clock( ) - Battle->lastAtkTime;
    int weapontype = IsPlayer()?getWeaponType():0;
    if (weapontype == 212)return true;
    if (0 == Stats->Attack_Speed) return false;

    if (Battle->lastcastspeed <= 0 || Battle->lastcastspeed == Stats->Attack_Speed) // aspd formula
    {
...
void CCharacter::RefreshBuff( )
{
    ...
    for( UINT i=0;i<32;i++)
    {
        ...
        if( (etime >= MagicStatus[i].Duration * CLOCKS_PER_SEC) || Status->Stance==DRIVING) //Terr0risT fix.
        {

            Log(MSG_INFO,"Magic Status1 %i, vanish after: %i", MagicStatus[i].Status, MagicStatus[i].Duration);
            ...
            switch(MagicStatus[i].Status)
            {
                ...
                case 33://Stealth,Camoflauge
                {
                    ...
                    Log(MSG_WARNING,"Skill status %u.",MagicStatus[i].Status);
                    Status->Stealth = 0xff;
                }
                break;
        ...
        {
             //Stats->HP -= MagicStatus[i].Status + Stats->Level; //Actually take 7, 8, 9, 10 or 11 from the health. Based on the Status itself, LMA: can be 89 noc too.
             int poisondmg = Stats->MaxHP * 1/100;
             Stats->HP -= poisondmg; // New formula by Terr0risT. Take 1% of MAX HP over time.
             ...
             //Log(MSG_WARNING,"did %i poison dmg to the player / monster, still %i seconds and %I64i HP remain", poisondmg, MagicStatus[i].Duration, Stats->HP);
        ...
        else if ( (MagicStatus[i].Status == 58 || MagicStatus[i].Status == 61 || MagicStatus[i].Status == 71 || (MagicStatus[i].Status >= 77 && MagicStatus[i].Status <= 80) || MagicStatus[i].Status == 88) && etime > 1*CLOCKS_PER_SEC) //Do flame dmg every 1.5 seconds
        {
             ...
             int flamedmg = Stats->MaxHP * 2/100;
             Stats->HP -= flamedmg; // Terr0risT New Formula
             MagicStatus[i].BuffTime += 1*CLOCKS_PER_SEC;
             MagicStatus[i].Duration -= 1;
             //Log(MSG_WARNING,"did %i poison dmg to the player / monster, still %i seconds and %I64i HP remain", flamedmg, MagicStatus[i].Duration, Stats->HP);
    ...
    for( UINT i=0;i<32;i++)
    {
        ...
        CPlayer* thisplayer = GServer->GetClientByID(clientid);
            Log(MSG_INFO,"Magic Status2 %i, vanish after: %i", MagicStatus2[i].Status, MagicStatus2[i].Duration);
...
// [VIRTUAL]
void CCharacter::reduceItemsLifeSpan(bool attacked)
{
}
