...
void CWorldServer::SetVisibleEquipment(CPlayer* thisclient, UINT slot)
{
	int tmpslot = 0;
	switch(slot)
	{
		case 1:{ tmpslot = 146;}break; //Face
		case 2:{ tmpslot = 142;}break; //Cap
		case 3:{ tmpslot = 143;}break; //Body
		case 4:{ tmpslot = 147;}break; //Back
		case 5:{ tmpslot = 144;}break; //Gloves
		case 6:{ tmpslot = 145;}break; //Shoes
		default:{ tmpslot = 0;}break;
	}

	if(thisclient->items[tmpslot].itemnum != 0 && thisclient->items[tmpslot].itemtype != 0)
	{
	...
	switch(slot)
	{
		case 146:{ tmpslot = 1;}break; //Face
		case 142:{ tmpslot = 2;}break; //Cap
		case 143:{ tmpslot = 3;}break; //Body
		case 147:{ tmpslot = 4;}break; //Back
		case 144:{ tmpslot = 5;}break; //Gloves
		case 145:{ tmpslot = 6;}break; //Shoes
		default:{ tmpslot = slot;}break;
	}
	int newslot = slot;
	if(thisclient->items[tmpslot].itemnum != 0 && thisclient->items[tmpslot].itemtype != 0)
	{
	...
	if(thisclient->items[slot].itemnum != 0 && thisclient->items[slot].itemtype != 0)
	{
		newslot = slot;
	}
	...
	ADDWORD(pak, tmpslot);
    ADDWORD ( pak, thisclient->items[newslot].itemnum );
    ADDWORD    ( pak, BuildItemRefine( thisclient->items[slot]));
...
unsigned CWorldServer::GetNewGAId( )
{
    //LMA: if the thread is already blocked, it should return EBUSY else 0
    UINT res_blockP=pthread_mutex_trylock(&GServer->PlayerMutex);
    UINT res_blockM=pthread_mutex_trylock(&GServer->MapMutex);
	for (unsigned i=last_gid; i<0xffff; i++)
    {
		if (GAIDList[i]!=0 && time(NULL)-GAIDList[i]>10)
        {
			GAIDList[i] = 0;
			//Log(MSG_INFO,"XCID NEW %u",i);
			last_gid=i+1;
...
unsigned CWorldServer::GetNewGAGroup( )
{
    UINT res_blockP=pthread_mutex_trylock(&GServer->PlayerMutex);
    UINT res_blockM=pthread_mutex_trylock(&GServer->MapMutex);
...
// This function will free our clientID
void CWorldServer::ClearGAid( unsigned int id )
{
    if(id<0||id>0xffffff)
    {
        return;
    ...
    {
        GAIDList[id] = (unsigned)time(NULL);
    }
...
{
    if (idorg > NPCData.size())
    {
        Log(MSG_WARNING,"STLNPCMONSTER, incorrect id %u>%u",idorg,NPCData.size());
...
{
    if (idorg>maxZone)//MapList.Index.size())
    {
...
char* CWorldServer::GetSTLObjNameByID(UINT family, UINT idorg,bool comment)
{
    ...
    if(family==15)
    {
        if(idorg>=MountList.max)
        {
            Log(MSG_WARNING,"STLObject, incorrect object %i::%i",family,idorg);
            return STLNameList[1];
        }

        id=MountList.Index[idorg]->STLId;
    }
...
{
    if (id>=StatusList.size())
    {
           Log( MSG_WARNING,"STATUS NOT FOUND! %i>=%i", id,StatusList.size());
           return NULL;
    }
...
CUseInfo* CWorldServer::GetUseItemInfo(CPlayer* thisclient, unsigned int slot )
{
    ...
    useitem->usevalue = 0;
    useitem->use_buff=0;
    useitem->cooldown_type=0;   //LMA: cooldown handling for foodfood.
        ...
        case 313://Magic Item [scrolls/summons/fireworks/etc]
        {
            ...
            if( (useitem->itemnum>79 && useitem->itemnum<93) ||
                ...
                (useitem->itemnum>1040 && useitem->itemnum<1047) ||
                 useitem->itemnum == 1305 || useitem->itemnum>= 1049 && useitem->itemnum<=1054 ||
                 useitem->itemnum == 205 || useitem->itemnum == 206 || useitem->itemnum == 208 )
            {
                useitem->usescript = 7;
                ...
                (useitem->itemnum>1029 && useitem->itemnum<1035) )
            {
                ...
                switch( useitem->itemnum )
                {
                    ...
                    case 350:
                        useitem->usetype = 22;
                        useitem->usevalue = 56515200; //Original AP
                    ...
                    case 360:
                        ...
                        useitem->usevalue = 57055481; //55185444 Value Before - rl2171
                    break;
                    ...
                    break;
                    //Junon's Order Return Scroll - Modified to Junon Cartel by Terr0risT
                    case 945:
                        useitem->usetype = 5;
                        useitem->usevalue = 52005222;
                    break;
                    case 1030: // Wreckage of The Blinding Mist - added by Terr0risT
                        useitem->usetype = 74;
                        useitem->usevalue = 51685249;
                    break;
                    case 1031: // Golden Ring 75 - added by Terr0risT
                        useitem->usetype = 75;
                        useitem->usevalue = 52755324;
                    break;
                    case 1032: // Golden Ring 76 - added by Terr0risT
                        useitem->usetype = 76;
                        useitem->usevalue = 54684976;
                    break;
                    case 1033: // Golden Ring 77 - added by Terr0risT
                        useitem->usetype = 77;
                        useitem->usevalue = 50735244;
                    break;
                    case 1034: // Ancient Oasis Shrine - added by Terr0risT
                        useitem->usetype = 80;
                        useitem->usevalue = 50185158;
                    break;
                }
            }
...
UINT CWorldServer::ReturnItemType( CPlayer* thisclient, int slot )
{
    ...
    else if(thisclient->items[material].itemtype==15)
    {
        if(thisclient->items[material].itemnum>=MountList.max)
        {
            Log(MSG_WARNING,"Incorrect itemnum in ReturnItemType slot %i, item %u::%u",thisclient->items[material].itemtype,thisclient->items[material].itemnum);
            return 0;
        }

        type=MountList.Index[thisclient->items[material].itemnum]->type;
    }
    else
    {
...
bool CWorldServer::ADDGAGroup( GAGroup* thisgroup )
{
    //LMA: adding ID.
    thisgroup->GroupId=GetNewGAGroup();
    Log(MSG_WARNING,"New groupid %i",thisgroup->GroupId);
    GroupList.push_back( thisgroup );
    return true;
}
...
bool CWorldServer::RemoveGroup( GAGroup* thisgroup )
{
    for(UINT i=0;i<GroupList.size( );i++)
    {
        if(GroupList.at(i)==thisgroup)
        {
            GroupList.erase(GroupList.begin( )+i );
            //LMA: freeing the ID as well.
            GAGroupIDList[thisgroup->GroupId]=false;
            return true;
        }
    }
    return false;
...
bool CWorldServer::SaveItemMallPoints(CPlayer* thisclient)
{
     ...
     GServer->DB->QExecute("update accounts set itemmall_points = %I64i where id = %i", thisclient->CharInfo->Itemmall, thisclient->Session->userid);
...
void CWorldServer::pakSetVisibleEquipment(CPlayer* thisclient, BYTE slot)
{
    int tmpslot = 0;
	switch(slot)
	{
    ...
    ADDWORD    ( pak, thisclient->clientid );
    ADDWORD    ( pak, slot );
    ADDWORD    ( pak, thisclient->items[slot].itemnum );
    ADDWORD    ( pak, BuildItemRefine( thisclient->items[slot]));
    ADDWORD    ( pak, thisclient->Stats->Move_Speed );
    SendToVisible( &pak, thisclient );
}
...
UINT CWorldServer::GetBreakID(UINT itemnum,UINT itemtype)
{
   ...
   if (breakid!=0)
    {
        if(breakid>=BreakList.size())
        {
            Log(MSG_WARNING,"The breakid %u for (%i::%i) is > %u",breakid,itemtype,itemnum,BreakList.size());
            return 0;
        }
...
int CWorldServer::CheckPatList(CPlayer* client)
{
    int check = 0;
        for (int k=135;k<140;k++)
        {
            if (client->items[k].itemnum>0)
                check++;
        }
        return check;
}

int CWorldServer::GetMileageFreeSlotOnline(CPlayer* client)
{
    int freeslot = -1;
    for (int i=0 ; i<MAX_ITEMMALL;i++)
    {
        if (client->itemmallitems[i].itemnum == 0 && client->itemmallitems[i].itemtype == 0)
        {
            freeslot = i;
            break;
        }
...
int CWorldServer::GetMileageFreeSlotOffline(char* name)
{
    int freeslot = -1;
    MYSQL_ROW row;
    MYSQL_RES *character = GServer->DB->QStore("SELECT account_name FROM characters WHERE char_name ='%s'", name);
    row = mysql_fetch_row(character);
    memcpy( name, row[0], 16);
    GServer->DB->QFree();
    MYSQL_RES *account = GServer->DB->QStore("SELECT id FROM accounts WHERE username ='%s'", name);
    row = mysql_fetch_row(account);
    unsigned int userid = atoi(row[0]);
    GServer->DB->QFree();
    for (int i = 0; i < MAX_ITEMMALL; i++ )
    {
    MYSQL_RES *result = GServer->DB->QStore("SELECT itemnum FROM mileage WHERE owner =%i AND slotnum = %i", userid, i);
    row = mysql_fetch_row(result);
        if (row == NULL)
        {
            freeslot = i;
...
CItem CWorldServer::CreateItem(int itemID, int itemType, int amount) // still needs to be edited - coded only for equipitems
{
    CItem newitem;
    newitem.Clear();
    newitem.itemtype = itemType;
    newitem.itemnum = itemID;
    newitem.count = amount;
    newitem.gem = 0;
    newitem.durability = GServer->STB_ITEM[newitem.itemtype-1].rows[newitem.itemnum][29];
    newitem.lifespan = 100;
    return newitem;
}

bool CWorldServer::GetIfCharExists(const char * name)
{
    MYSQL_ROW row;
	MYSQL_RES *charname = GServer->DB->QStore("SELECT char_name FROM characters WHERE char_name ='%s'", name );
	row = mysql_fetch_row(charname);
	if (row != NULL)
    {
	if (strnicmp(name,row[0], 16)==0)
    {
    DB->QFree();
    return true;
    }
...
bool CWorldServer::GiftOffline( CPlayer* thisclient, char* name, int fromslot, int toslot)
{
    MYSQL_ROW row;
    MYSQL_RES *account = GServer->DB->QStore("SELECT id FROM accounts WHERE username ='%s'", name);
    row = mysql_fetch_row(account);
    ...
    int i = fromslot;
        GServer->DB->QExecute("INSERT INTO mileage (owner,slotnum,itemnum,itemtype,refine,durability,lifespan,count,stats,socketed,appraised,gem) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i) ON DUPLICATE KEY UPDATE owner=VALUES(owner),slotnum=VALUES(slotnum),itemnum=VALUES(itemnum),itemtype=VALUES(itemtype),refine=VALUES(refine),durability=VALUES(durability),lifespan=VALUES(lifespan),count=VALUES(count),stats=VALUES(stats),socketed=VALUES(socketed),appraised=VALUES(appraised),gem=VALUES(gem)",
    						userid, toslot, thisclient->itemmallitems[i].itemnum, thisclient->itemmallitems[i].itemtype,thisclient->itemmallitems[i].refine, thisclient->itemmallitems[i].durability,
    						thisclient->itemmallitems[i].lifespan, thisclient->itemmallitems[i].count, thisclient->itemmallitems[i].stats, (thisclient->itemmallitems[i].socketed?1:0),
                            (thisclient->itemmallitems[i].appraised?1:0), thisclient->itemmallitems[i].gem );
        GServer->DB->QExecute("DELETE FROM mileage WHERE owner=%i AND slotnum=%i", thisclient->Session->userid,fromslot);
        thisclient->itemmallitems[i].Clear();

    return true;
}

bool CWorldServer::GiftOnline( CPlayer* fromclient, CPlayer* toclient, int fromslot, int toslot)
{
    int i = fromslot;
        GServer->DB->QExecute("INSERT INTO mileage (owner,slotnum,itemnum,itemtype,refine,durability,lifespan,count,stats,socketed,appraised,gem) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i) ON DUPLICATE KEY UPDATE owner=VALUES(owner),slotnum=VALUES(slotnum),itemnum=VALUES(itemnum),itemtype=VALUES(itemtype),refine=VALUES(refine),durability=VALUES(durability),lifespan=VALUES(lifespan),count=VALUES(count),stats=VALUES(stats),socketed=VALUES(socketed),appraised=VALUES(appraised),gem=VALUES(gem)",
    						toclient->Session->userid, toslot, fromclient->itemmallitems[i].itemnum, fromclient->itemmallitems[i].itemtype,fromclient->itemmallitems[i].refine, fromclient->itemmallitems[i].durability,
    						fromclient->itemmallitems[i].lifespan, fromclient->itemmallitems[i].count, fromclient->itemmallitems[i].stats, (fromclient->itemmallitems[i].socketed?1:0),
                            (fromclient->itemmallitems[i].appraised?1:0), fromclient->itemmallitems[i].gem );

    toclient->itemmallitems[toslot].itemnum = fromclient->itemmallitems[i].itemnum;
    toclient->itemmallitems[toslot].itemtype = fromclient->itemmallitems[i].itemtype;
    toclient->itemmallitems[toslot].refine = fromclient->itemmallitems[i].refine;
    toclient->itemmallitems[toslot].durability = fromclient->itemmallitems[i].durability;
    toclient->itemmallitems[toslot].lifespan = fromclient->itemmallitems[i].lifespan;
    toclient->itemmallitems[toslot].count = fromclient->itemmallitems[i].count;
    toclient->itemmallitems[toslot].stats = fromclient->itemmallitems[i].stats;
    toclient->itemmallitems[toslot].socketed = fromclient->itemmallitems[i].socketed;
    toclient->itemmallitems[toslot].appraised = fromclient->itemmallitems[i].appraised;
    toclient->itemmallitems[toslot].gem = fromclient->itemmallitems[i].gem;

        GServer->DB->QExecute("DELETE FROM mileage WHERE owner=%i AND slotnum=%i", fromclient->Session->userid,i);
        fromclient->itemmallitems[i].Clear();
    return true;
}

bool CWorldServer::GetIfOwnName(CPlayer* thisclient, char* name)
{
    MYSQL_ROW row;
    MYSQL_RES *character = GServer->DB->QStore("SELECT account_name FROM characters WHERE char_name ='%s'", name);
    row = mysql_fetch_row(character);
	if (row != NULL)
    {
	if (strnicmp(thisclient->Session->username,row[0], 16)==0)
    {
    DB->QFree();
    return true;
    }
