...
bool CWorldServer::pakGMCommand( CPlayer* thisclient, CPacket* P )
{
   ...
   if(strcmp(command, "addquest")==0)
    ...
    else if (strcmp(command, "freerose")==0)
    {
        if(Config.Command_freeroseBuff > thisclient->Session->accesslevel && thisclient->Stats->Level > 100)
           return true;
        ...
        if(thisclient->CharInfo->LastGlobal+Config.Command_freeroseTime <= seconds && thisclient->CharInfo->Zulies >= levelfee && thisclient->Stats->Level <= 100)
        {
                    thisclient->CharInfo->LastGlobal = time (NULL);
                    strength = 200;
                    pakGMBuff(thisclient, strength);

                    char freerosemsg[200];
                    sprintf( freerosemsg, "I voted online today! FreeROSE is the best Evo Private Server!");
                    SendGlobalMSG(thisclient, freerosemsg);

            if(thisclient->Session->accesslevel == 100)
			{
				char deduct[200];
				thisclient->CharInfo->Zulies = thisclient->CharInfo->Zulies - levelfee;
				sprintf (deduct, "You paid %i zulie for buff TAX!", levelfee);
				SendSysMsg(thisclient, deduct);
				BEGINPACKET( pak, 0x71d );
                ADDQWORD( pak, thisclient->CharInfo->Zulies );
                thisclient->client->SendPacket( &pak );
        ...
        {
            long int remaining = (Config.Command_freeroseTime - (seconds - thisclient->CharInfo->LastGlobal));
            char buffer2[200];
            char buffer3[200];
			char zulieneed[200];
            ...
            {
				sprintf (buffer3, "Oh no, still weak? Please find a Cleric or use your own buff skills.");
                SendPM(thisclient, buffer3);
            }
            else if(thisclient->CharInfo->Zulies < levelfee)
            {
                sprintf (zulieneed, "You need to pay %i zulie.", levelfee);
                SendSysMsg(thisclient, zulieneed);
            }
            else
            {
                sprintf ( buffer2, "Please wait for %i seconds before requesting for a /freerose buff again.", remaining );
                SendPM(thisclient, buffer2);
            }
        }
        return true;
    ...
    else if (strcmp(command, "cart")==0)     // get all castlegear parts
    {
        ...
        BEGINPACKET( pak, 0);
        {
            ...
            while( row = mysql_fetch_row(result) )
            {
                ...
                thisclient->UpdateInventory( good_slot,0xffff,false);

                pakSetVisibleEquipment(thisclient, good_slot);
            }
   ...
   else if (strcmp(command, "cha")==0)
    {
	   ...
	   pakSetVisibleEquipment(thisclient, slot);
...
//		ADDSTRING( pak, "DELETESPAWN" );
...
else if(strcmp(command, "gmlist")==0) /* GM List {By CrAshInSiDe} */
    {
        ...
        while(count <= (ClientList.size()-1))
        {
            ...
            if(whoclient->Session->accesslevel > 201)
            {
   ...
   else if (strcmp(command, "gmgear")==0)     // get all GM Gear
    {
        if(Config.Command_GMGear > thisclient->Session->accesslevel)
           return true;
        Log( MSG_GMACTION, " %s : /gmgear", thisclient->CharInfo->charname);
        BEGINPACKET( pak, 0);
        {
            MYSQL_ROW row;
            MYSQL_RES *result=NULL;
            result = DB->QStore("SELECT itemnumber, itemtype FROM list_gm_gear WHERE GEAR='GM' AND isactive=1");
            if(result==NULL) return true;
            ...
            while( row = mysql_fetch_row(result) )
            {
                ...
                if(!is_ok)
                {
                    SendPM(thisclient,"Not enough place in inventory!");
                    break;
                }

                thisclient->items[good_slot].itemnum = itemnum;
                thisclient->items[good_slot].itemtype = itemtype;
                thisclient->items[good_slot].refine = 240;  // refine 15 should be 15*16 = 240
                thisclient->items[good_slot].durability = 100;
                thisclient->items[good_slot].lifespan = 100;
                thisclient->items[good_slot].count = 1;
                thisclient->items[good_slot].stats = 0;
                thisclient->items[good_slot].socketed = false;
   ...
   else if (strcmp(command, "here")==0)
	{
        ...
        if ( thisclient->Stats->HP < 1 || thisclient->Session->inGame == false )
            {
                    SendPM(thisclient, "You are already DEAD. You cannot use this command.");
                    return true;
            }
        if ( thisclient->Position->Map == 5 || thisclient->Position->Map == 6 )
            {
                    return true;
            }
...
//        if (i > 0 && i%10 == 0) {
   ...
   else if(strcmp(command, "whatis")==0)    // mystat - by PurpleYouko
    {
        if ((tmp = strtok(NULL, " "))==NULL)
        {
   ...
   else if(strcmp(command, "whatiz")==0)    // whatiz by Terr0risT - from osprose to test
    {
         if ((tmp = strtok(NULL, " "))==NULL)return true;
         if(strcmp(tmp, "ap")==0)
         {
         ...
         {
              SendPM(thisclient, "My attack speed percent is %f", thisclient->Stats->Attack_Speed_Percent);
         }
...
//         }
   ...
   else if (strcmp(command, "set")==0)
    {
        ...
        if(EquipList[2].Index[id]!=NULL)
        {
    	    ...
    	    pakSetVisibleEquipment(thisclient, 2);
        }
    	if(EquipList[3].Index[id]!=NULL)
    	{
	        ...
	        pakSetVisibleEquipment(thisclient, 3);
        }
        if(EquipList[4].Index[id]!=NULL)
        {
    	    ...
    	    pakSetVisibleEquipment(thisclient, 5);
        }
        if(EquipList[5].Index[id]!=NULL)
        {
    	    ...
    	    pakSetVisibleEquipment(thisclient, 6);
        }
        thisclient->SetStats( );
    }
   ...
   else if (strcmp(command, "shutdown")==0)
	{
        ...
        char text[200];
        sprintf( text, "FreeROSE SERVER will shutdown and reboot in %i minute(s), Please logout NOW to be sure your information is saved correctly.",
           minutes );
        BEGINPACKET( pak, 0x702 );
    ...
    else if (strcmp(command, "happyhour")==0) // show hh map
    {
		...
		{
			SendPM(thisclient, "You will receive DOUBLE exp on map - %s", GServer->Config.mapName.c_str( ));
		}else
		{
...
bool CWorldServer::pakGMExportSTBSTL(CPlayer* thisclient)
{
    ...
    //Quest items.
    for (int j=0;j<QuestItemData.size();j++)
    {
