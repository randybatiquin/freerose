...
long int CPlayer::ReturnPvp( CPlayer* player, CPlayer* otherclient )
{
    ...
    switch (pvp_status)
    {
        ...
        case 0:
        {
            ...
            Log(MSG_WARNING,"%s, pvp map (%i), we send pvp_id==%i (everyone is enemy)",CharInfo->charname,map->id,clientid + 0x100);
            return (clientid + 0x100);
        }
        break;
        ...
        {
            //clan is friendly
            Log(MSG_WARNING,"%s, pvp map (%i), we send pvp_id==%i (friendly clan)",CharInfo->charname,map->id,Clan->clanid);
            //return Clan->clanid;
...
// End 434
    ...
    /*if( Group->group != NULL && Group->group == player->Group->group)
    {
        BEGINPACKET( pak, 0x7d5 );
        ADDDWORD   ( pak, CharInfo->charid );
        ADDWORD    ( pak, clientid );
