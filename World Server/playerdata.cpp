...
{
	MYSQL_ROW row;
	MYSQL_RES *itemmall = GServer->DB->QStore("SELECT itemmall_points FROM accounts WHERE id='%i'", Session->userid );
	row = mysql_fetch_row(itemmall);
	CharInfo->Itemmall = atoi(row[0]);
	GServer->DB->QFree( );

	MYSQL_RES *result = GServer->DB->QStore("SELECT level,face,hairStyle,sex,classid,zuly,str,dex,_int, con,cha,sen,\
                                         curHp,curMp,id,statp,skillp,exp,stamina,quickbar,basic_skills, class_skills,class_skills_level,\
                                         respawnid,clanid,clan_rank,townid,rewardpoints,unionid,unionfame,union01,union02,union03,union04,\
                                         union05,bonusxp,timerxp,shoptype,timershop,isGM,unique_skills,mileage_skills,driving_skills,\
                                         unique_skills_level,mileage_skills_level,bonusddrop,timerddrop,bonusstatdrop,timerstatdrop,bonusgraydrop,\
                                         timergraydrop,hairColor,honor,valor,premium,shuffles,spins,monsters_killed FROM characters WHERE char_name='%s'", CharInfo->charname);
	...
	CharInfo->HairColor = atoi(row[51]);
    ...
    CharInfo->Honor = atoi(row[52]);
	CharInfo->Valor = atoi(row[53]);
	CharInfo->Premium = atoi(row[54]);
	CharInfo->LuckyShuffles = atoi(row[55]);
	CharInfo->LuckySpins = atoi(row[56]);
	CharInfo->MonstersKilled = atoll(row[57]);   // Monsters Kill count by Terr0risT
...
void CPlayer::savedata( )
{
    ...
    if(Session->userid!=0)
    {
	   ...
	   if(Stats->MP<0)
        ...
        // Saving Item Mall points.. - Terr0risT
        GServer->DB->QExecute("UPDATE accounts set itemmall_points = %I64i where id = %i", CharInfo->Itemmall, Session->userid);

        GServer->DB->QExecute("UPDATE characters SET classid=%i,level=%i,zuly=%I64i,curHp=%i,curMp=%i,str=%i,con=%i,\
                              dex=%i,_int=%i,cha=%i,sen=%i,exp=%i,skillp=%i,statp=%i, stamina=%i,quickbar='%s',respawnid=%i,\
                              clanid=%i,clan_rank=%i, townid=%i, rewardpoints=%i, bonusxp=%i, timerxp=%i, shoptype=%i,\
                              timershop=%i, unionid=%i, unionfame=%i, union01=%i, union02=%i, union03=%i, union04=%i, union05=%i,\
                               bonusddrop=%i, timerddrop=%i, bonusstatdrop=%i, timerstatdrop=%i, bonusgraydrop=%i,\
                               timergraydrop=%i, hairColor=%i,honor=%i,valor=%i,premium=%i,shuffles=%i,spins=%i,monsters_killed=%I64i WHERE id=%i",
                    ...
                    temp_ddrop,temp_timer_ddrop,temp_statdrop,temp_timer_statdrop,temp_graydrop,temp_timer_graydrop,
                    CharInfo->HairColor, CharInfo->Honor, CharInfo->Valor, CharInfo->Premium, CharInfo->LuckyShuffles, CharInfo->LuckySpins, CharInfo->MonstersKilled, CharInfo->charid);

        //Storage Expansion Save
        GServer->DB->QExecute("UPDATE accounts SET ExpansionTime='%i', itemmall_points='%i' WHERE username = '%s'",timerexapnsion,CharInfo->Itemmall,Session->username);
...
{
    if (skill_id>GServer->SkillList.size())
    {
        Log(MSG_WARNING,"Bad skill_id in GoodSkill (%i>%u)",skill_id,GServer->SkillList.size());
        return -1;
    }
