...
UINT CPlayer::GetLevelEXP( )
{
	...
	else if (Stats->Level <= 230) return (unsigned int)( ( Stats->Level - 126 ) * ( Stats->Level - 15 ) * ( Stats->Level + 7 ) * 41 );
	else if (Stats->Level <= 250) return (unsigned int)( 217272120 );
	else return (unsigned int)( 1086360600 ); // 500% on EXP Bar
}
...
bool CPlayer::CheckPlayerLevelUP( )
{
	...
	while (CharInfo->Exp >= GetLevelEXP( ) && Stats->Level < GServer->Config.MaxLevel)
    {
	    ...
	    if(Stats->Level >= 10) CharInfo->SkillPoints += 1;
	    // April Valor Event - FreeROSE by Terr0risT.
	    if(Stats->Level > 100)
	    {
	        CharInfo->Valor += 10;  // 10 Valor Points
	    ...
	    if(Stats->Level > 200)
	    {
	        CharInfo->Zulies += 1000000;  // 1 Million Zulie
	        BEGINPACKET( pak3, 0x71d );  // Send zulie update packet - Terr0risT
            ADDQWORD( pak3, CharInfo->Zulies );
            client->SendPacket( &pak3 );
	    }
	    if(Stats->Level > 240)
	    {
	        CharInfo->LuckySpins += 1; // 1 Spin
	        BEGINPACKET ( pak4, 0x7f0 ); // Send luckyspins update packet - Terr0risT
            ADDBYTE     ( pak4, 1 );
            ADDDWORD    ( pak4, CharInfo->LuckySpins );
...
bool CPlayer::ForceRefreshMonster(bool refresh_all, UINT monster_id)
{
    ...
    for(UINT i=0;i<map->MonsterList.size();i++)
    {
        ...
        if(map->id >= 130 && map->id <= 150)
        {
            if(this->Groupga->Groupga->GroupId!=thismon->team)
            {
                continue;
            }
        }
		...
		if ( GServer->IsVisible( this, thismon ) )
        {
			...
			{
                ClearObject( thismon->clientid );  //we clear it
                thismon->SpawnMonster(this, thismon );  //and respawn it
...
bool CPlayer::VisiblityList( )
{
    ...
    for(UINT i=0;i<map->PlayerList.size();i++)
    {
        ...
        if(map->id >= 130 && map->id <= 150)
        {
            if(this->Groupga->Groupga->GroupId!=otherclient->Groupga->Groupga->GroupId)
            {
                continue;
            }
        }
	...
	for(unsigned i=0; i<map->DropsList.size(); i++)
    {
		...
		if(map->id >= 130 && map->id <= 150)
        {
            if(this->Groupga->Groupga->GroupId!=thisdrop->team)
            {
                continue;
            }
        }
	...
	for(unsigned i=0; i<map->NPCList.size(); i++)
    {
		...
		if(map->id >= 130 && map->id <= 166)
        {
            if(this->Groupga->Groupga->GroupId!=thisnpc->team)
            {
                continue;
            }
        }
...
bool CPlayer::Regeneration( )
{
    ...
    if ((Stats->MaxHP == Stats->HP) && (Stats->MaxMP==Stats->MP))
     ...
     int bonus_hp = 0;
     int bonus_mp = 0;
     int nb_sec_stance = 8;   //old setting = 5

     if(Fairy)
     {
     nb_sec_stance = 8;      //old setting = 3
     bonus_mp++;
     bonus_hp++;
     bonus_fairy = 3;         //old setting = 3
     }
     if(Status->Stance==1)
     {
     nb_sec_stance = 2;      //new setting = 3
     bonus_mp++;
     bonus_hp++;
     bonus_sitted = 3;        //old setting = 3
     }

     int TimeDiff = nb_sec_stance*CLOCKS_PER_SEC;

    //LMA: HPregen
    if(Stats->HP < Stats->MaxHP && Battle->contatk==false)
    {
    ...
    if(Stats->MP < Stats->MaxMP)
    {
        ...
        if( etimeMP >= TimeDiff && Stats->MP >= 0 )
        {
...
bool CPlayer::PlayerHeal()
{
	...
	if(UsedItem->usevalue!=0 && transtime >= 1150)
    {
...
// return party pointer
GAGroup* CPlayer::GetGroup( )
{
    return Groupga->Groupga;
}
...
{
    return ( Attr->Int + Attr->Eint + (int)Attr->MEint + Attr->ALLbuff + Attr->INTbuff); //Tomiz : We need to take int from cloth,gem,ect into consideration(ExtraInt/Eint)
}

unsigned int CPlayer::GetDex( )
{
    return ( Attr->Dex + Attr->Edex + (int)Attr->MEdex + Attr->ALLbuff + Attr->DEXbuff); //Tomiz : We need to take int from cloth,gem,ect into consideration(ExtraInt/Eint)
}

unsigned int CPlayer::GetStr( )
{
    return ( Attr->Str + Attr->Estr + (int)Attr->MEstr + Attr->ALLbuff + Attr->STRbuff); //Tomiz : We need to take int from cloth,gem,ect into consideration(ExtraInt/Eint)
}

unsigned int CPlayer::GetCha( )
{
    return ( Attr->Cha + Attr->Echa + (int)Attr->MEcha + Attr->ALLbuff + + Attr->CHAbuff); //Tomiz : We need to take int from cloth,gem,ect into consideration(ExtraInt/Eint)
}

// return Sensibility
unsigned int CPlayer::GetSen( )
{
    return ( Attr->Sen + Attr->Esen + (int)Attr->MEsen + Attr->ALLbuff + Attr->SENbuff); //Tomiz : We need to take sen from cloth,gem,ect into consideration(ExtraSen/Esen)
}

// return Concentration
unsigned int CPlayer::GetCon( )
{
    return (Attr->Con + Attr->Econ + (int)Attr->MEcon + Attr->ALLbuff + Attr->CONbuff); //Tomiz : We need to take con from cloth,gem,ect into consideration(ExtraCon/Econ)
}

unsigned int CPlayer::GetClassRequiredStatus()
{
	switch(this->CharInfo->Job)
	{
		case CLASS_VISITOR:
		{
			return 0;
...
void CPlayer::UpdateCostume( unsigned int slot1, unsigned int slot2, bool save)
{
    if(slot1 == 0xffff && slot2==0xffff) return;
            ...
            if ( (i!=7 && attacked) || ( i==7 && (!attacked) ) )
            {
             ...
             if (items[i].itemnum != 0 && items[i].appraised )
             {
                ...
                if (items[i].lifespan < 1)
                {
                   ...
                   GServer->pakSetVisibleEquipment(this, i);
                }
                   UpdateInventory(i,0xffff, false);         //LMA: will be saved in database at auto save.
             }
            }
        }
}
...
bool CPlayer::CheckDoubleEquip()
{
    ...
    switch(weapontype)
    {
        ...
        case DUAL_GUN:
        {
            ...
            GServer->pakSetVisibleEquipment(this, 8);
            SetStats( );

            //resetting quickbar
            Log(MSG_HACK,"%s tryed to use a shield with a double hand weapon",CharInfo->charname);
            GServer->SendPM(this,"<GM> Don't hack the game !!! You are now reported to admin Terr0risT!!!");
            /*
...
int CPlayer::ExecuteQuestTrigger(dword hash,bool send_packet, UINT index)
{
    ...
    for(unsigned j=index; j < GServer->TriggerList.size(); j++)
    {
      if (GServer->TriggerList.at(j)->TriggerHash == hash)
      {
        ...
        Log(MSG_DEBUG, "trigger = %u", trigger);
    ...
    int success = QUEST_SUCCESS;
      //Log(MSG_WARNING, "EXTP::Trigger Executed: %s[%i]", trigger->TriggerName, trigger->CheckNext);
    if(GServer->ServerDebug)
    ...
    for (dword i = 0; i < trigger->ConditionCount; i++)
    {
      ...
      if (success == QUEST_FAILURE)
      {
        ...
        {
            Log(MSG_WARNING, "EXTP::checknext because FAILURE");
            if(GServer->ServerDebug)
        ...
        Log(MSG_WARNING, "EXTP::Quest cdt success");
      if(GServer->ServerDebug)
    ...
    for (dword i = 0; i < trigger->ActionCount; i++)
    {
      ...
      Log(MSG_ERROR, "EXTP::Reward %03u returned %d, %i / %i", command, (*GServer->qstRewdFunc[command])(GServer, this, trigger->Actions[i]->data),i+1,trigger->ActionCount);
      }
    //LMA: In some case we have to send a quest packet.
    if(send_packet&&success==QUEST_SUCCESS)
    {
        if(GServer->ServerDebug)
            Log(MSG_WARNING,"QSD returned success. Sending 0x730 for questid %u",hash);
...
void CPlayer::SendQuestUpdate()
{
    ...
    for( unsigned i = 0; i < 8; i++ )
        ADDBYTE( pak, 0x00 );
    client->SendPacket(&pak);
    //Log(MSG_WARNING,"Sending packet");
}
