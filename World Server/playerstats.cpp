    ...
    for(UINT i=1;i<9;i++)               //Dodge from Item Refine
    {
        ...
        if(items[i].count>0)
        {
            ...
            if(items[i].refine>0)
            {
                ...
                if(refine>0 && refine<GServer->GradeList.size() && items[i].itemtype!=9)
                {
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(items[i].stats>0 && items[i].stats<GServer->StatsList.size() && items[i].gem==0)
                {
                    for(UINT j=0;j<2;j++)
                    {
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        ...
        else if(items[7].count !=0)
        {
            ...
            if(items[7].refine>0)
            {
                ...
                if(refine>0 && refine<GServer->GradeList.size())
                {
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        ...
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(items[i].stats>0 && items[i].stats<GServer->StatsList.size())
                {
                    for(UINT j=0;j<2;j++)
                    {
...
{
    UINT aspeed = GServer->EquipList[WEAPON].Index[items[7].itemnum]->attackspeed;
    ...
    for(UINT i=0;i<MAX_ALL_SKILL;i++)
	{
		...
		if( cskills[i].thisskill->type == 15 ) //Pasive
		{
			for( UINT wep=0;wep<5;wep++)
			{
				if((GServer->EquipList[WEAPON].Index[items[7].itemnum]->type == cskills[i].thisskill->weapon[wep] && cskills[i].thisskill->weapon[wep] != 0) || cskills[i].thisskill->weapon[0] == 0)
				{
					for( UINT j=0;j<3;j++ )
					{
						if(cskills[i].thisskill->buff[j] == ATTACK_SPEED)
						{
							if( cskills[i].thisskill->value2[j] > 0 )
							{
								skillas += (float)((aspeed*cskills[i].thisskill->value2[j])/1000.f);
							}

							if( cskills[i].thisskill->value1[j] > 0 )
							{
								skillas += (float)cskills[i].thisskill->value1[j];
							}
						}
					}
				}
    ...
    for(UINT i=1;i<12;i++)//cloth stats [from tomiz]
    {
        if( items[i].count != 0 )
        {
            ...
            if(items[i].gem!=0)
            {
                if(GServer->JemList.Index[items[i].gem]!=NULL)
                {
                    if( GServer->JemList.Index[items[i].gem]->stat1[0] == ATTACK_SPEED)
                    {
                        ...
                        if(GServer->JemList.Index[items[i].gem]->stat1[1] != 0)
                        aspeed += (float)(aspeed/100) * GServer->JemList.Index[items[i].gem]->stat1[1];
                    }
            ...
            if(items[i].stats>0 && items[i].stats<301)
            {
	...
	this->Stats->Base_Attack_Speed = aspeed;
    ...
    Log(MSG_INFO, "Attack Speed is %i", aspeed);
...
float CPlayer::GetAttackSpeedPercent()
{
    ...
    if(Status->Haste_up!=0xff)
    {
        Log(MSG_HACK, "Before BUFF Attack Speed: %f", paspeed);
        paspeed += MagicStatus[Status->Haste_up].Value;
        Log(MSG_HACK, "After BUFF Attack Speed: %f", paspeed);
    }
    if(Status->Haste_down!=0xff)
    {
    ...
    //Log(MSG_HACK, "Percent Attack Speed: %f", paspeed);
    ...
    switch(Status->Stance)
    {
        case WALKING: //walking
        {
            ...
            if(Status->Stance == MOUNTED)
                mspeed = GServer->MountList.Index[CharInfo->Mount]->wspeed;
        }
        break;
        case RUNNING: // running
        {
                ...
                if(items[i].gem!=0)
                {
                    if(GServer->JemList.Index[items[i].gem]!=NULL)
                    {
                        if( GServer->JemList.Index[items[i].gem]->stat1[0] == A_DASH)
                        {
                            ...
                            if(GServer->JemList.Index[items[i].gem]->stat1[1] != 0)
                            mspeed += (float)(mspeed/100) * GServer->JemList.Index[items[i].gem]->stat1[1];
                        }
            ...
            for(UINT i=0;i<MAX_ALL_SKILL;i++)
            {
                ...
                if( cskills[i].thisskill->type == 15 ) //Pasive
                {
                    for( UINT j=0;j<3;j++ )
                    {
                        ...
                        {
                            if( cskills[i].thisskill->value2[j] > 0 )
                                mspeed += (float)mspeed * cskills[i].thisskill->value2[j] / 100;
                            if( cskills[i].thisskill->value1[j] > 0 )
                            {
            ...
            if(Status->Stance == MOUNTED)
            {
                mspeed = GServer->MountList.Index[CharInfo->Mount]->mspeed;
            }
...
unsigned long long CPlayer::GetMaxHP( )
{
    ...
    switch(CharInfo->Job)
    {
        ...
        case 321:case 322:
        case 421:case 422:
            hpmax += 500; // Original value is 300. Fix by Terr0risT
        break;
        case 121:case 122:
            hpmax += 1000; // Original value is 300. Fix by Terr0risT
        break;
    }
...
unsigned int CPlayer::GetMaxWeight( )
{
    ...
    weight = ((Attr->Str + Attr->Estr) + (Attr->Str + Attr->Estr)*2) * 2 + Stats->Level+250 + (Stats->Level+250)*4;
