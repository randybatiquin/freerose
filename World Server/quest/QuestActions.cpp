...
QUESTREWD(000)
{
	...
	switch(data->btOp){//0 remove, 1 start, 2 replace quest keep items, 3 replace quest delete items, 4 select
		case 0:
		{
            if (client->ActiveQuest == data->iQuestSN) //we found the active quest
            {
                client->ActiveQuest = 0;        //clear active quest
            }

            for(dword i = 0; i < 10; i++)
            {
                if(client->quest.quests[i].QuestID != data->iQuestSN) continue;
                //found the quest we wanted
                Log(MSG_DEBUG,"Removing quest %i",data->iQuestSN);
                client->SendQuestUpdate();    //send the 723 BEFORE deleting quest data from the client
                BEGINPACKET ( pak, 0x730);    //send the 730 to get quest text at client. Not sure what it does in this case
                ADDBYTE     ( pak, 0x01 );    //always seems to be a 1
                ADDBYTE     ( pak, i );       //slot where we put the new quest
                ADDDWORD    ( pak, 0 );       //fillers??
                ADDDWORD    ( pak, data->iQuestSN ); //Quest id. NOT hash
                client->client->SendPacket(&pak);

                //PY More lmame special case code. Leave it be for now since UW doesn't exist presently
                //LMA: Special for UW :(
                ...
                memset(&client->quest.quests[i], 0, sizeof(SQuest));        //delete quest data from client
		...
		case 1:     // start new quest
		{
			...
			for(dword i = 0; i < 10; i++)
			{
                ...
                Log(MSG_DEBUG,"starting quest %i",data->iQuestSN);
                client->SendQuestUpdate();    //send the 723 AFTER updating quest data for the client
                BEGINPACKET ( pak, 0x730);    //send the 730 to get quest text at client
                ADDBYTE     ( pak, 0x01 );    //always seems to be a 1
                ADDBYTE     ( pak, i );       //slot where we put the new quest
                ADDDWORD    ( pak, 0 );       //fillers??
                ADDDWORD    ( pak, data->iQuestSN ); //Quest id. NOT hash
		...
		break;
		case 2:     //replace quest with a new one.
		{
            if( client->questdebug )
            ...
            for(dword i = 0; i < 10; i++)
            {
                ...
                Log(MSG_DEBUG,"Replacing quest with %i. keep items",data->iQuestSN);
                client->SendQuestUpdate();    //send the 723 AFTER updating quest data for the client
                BEGINPACKET ( pak, 0x730);    //send the 730 to get quest text at client
                ADDBYTE     ( pak, 0x01 );    //always seems to be a 1
                ADDBYTE     ( pak, i );       //slot where we put the new quest
		...
		break;
		case 3:     //replace quest with a new one
		{
            if( client->questdebug )
            ...
            for(dword i = 0; i < 10; i++)
            {
				...
				Log(MSG_DEBUG,"Replacing quest with %i. delete items",data->iQuestSN);
                client->SendQuestUpdate();    //send the 723 AFTER updating quest data for the client
                BEGINPACKET ( pak, 0x730);    //send the 730 to get quest text at client
                ADDBYTE     ( pak, 0x01 );    //always seems to be a 1
                ADDBYTE     ( pak, i );       //slot where we put the new quest
		...
		case 4:
		{
			...
			client->ActiveQuest = data->iQuestSN;
			Log(MSG_DEBUG,"Selecting quest %i",data->iQuestSN);
		}
		break;
	}
	return QUEST_SUCCESS;
...
QUESTREWD(003)
{
    ...
    if( client->questdebug )
      server->SendPM(client, "Update stat %i - %i (Op: %i)", curAbil->iType, curAbil->iValue, curAbil->btOp);
        ...
        case 3:
        {
            if(!OperateValues<int>(curAbil->btOp,(int*)&client->CharInfo->HairColor,curAbil->iValue))
                return QUEST_FAILURE;
            LogDebug("QSDA3 setting hairColor to %i", client->CharInfo->HairColor);

            BEGINPACKET( pak, 0x721 );
            ADDWORD ( pak, 3 );
            ADDWORD ( pak, client->CharInfo->HairColor );
            ADDWORD ( pak, 0 );
            client->client->SendPacket( &pak );
            ...
            ADDWORD ( pak, 0x40b3 );
            client->client->SendPacket( &pak );
            GServer->DB->QExecute("UPDATE characters SET hairColor=%i WHERE id=%i",client->CharInfo->HairColor,client->CharInfo->charid);
        }
        break;
        case sFace:
        {
	...
	switch(data->btTarget){
		case 0://EXP
		{
			if(!client->no_exp && client->Stats->Level < GServer->Config.MaxLevel)
            {
                ...
                {
                    client->CharInfo->Exp += GetRewardValue(data->btEquation, data->iValue, client, 0) * GServer->Config.doubleExpAmount;
                }else
                {
...
QUESTREWD(009){
  ...
  if( client->questdebug )
  {
    ...
    if(GServer->ServerDebug)
        Log(MSG_DEBUG,"Executing Quest Trigger QUESTREWD(009)");
...
QUESTREWD(013)
{
    ...
    int tempval=client->quest.RefNPC;
    if(GServer->ServerDebug)
        Log(MSG_DEBUG,"Executing Quest Trigger QUESTREWD(013)");
    int is_ok=client->ExecuteQuestTrigger(hash,true);
    client->quest.RefNPC=tempval;
...
QUESTREWD(020)
{
    ...
    client->pvp_id=-1; // Original Value is -1 Terr0risT
...
{
 	GETREWDDATA(029);
 	//word luaLen                 pos 0x00
	//string luaName              pos 0x02
	char* tempName = reinterpret_cast<char*>(&data->luaName) - 2;
    if( client->questdebug )
    {
        server->SendPM(client, " RWD 029 Execute LUA Trigger %s[%d] in client", tempName, data->luaLen);
    }

    if(GServer->ServerDebug)
        Log(MSG_DEBUG,"Executing Quest Trigger QUESTREWD(029)");
    //make the packet and send it
    BEGINPACKET( pak, 0x7f8 );
    ADDSTRING  ( pak, tempName );
    ADDBYTE    ( pak, 0x00 );
    client->client->SendPacket(&pak);
 	return QUEST_SUCCESS;
}
