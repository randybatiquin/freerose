	...
	for(dword i = 0; i < 10; i++)
	{
		if(client->quest.quests[i].QuestID == data->iQuestSN)
		{
            ...
            if( client->questdebug )
                server->SendPM(client, "Check Quest. Quest %i found", data->iQuestSN);
            return QUEST_SUCCESS;
        }
	}
    if( client->questdebug )
        server->SendPM(client, "Check Quest. Quest %i NOT found", data->iQuestSN);
...
QUESTCOND(001)
{
	...
	for(int i = 0; i < data->iDataCnt; i++)
	{
        ...
        if( client->questdebug )
          server->SendPM(client, "Check Quest Var. [%i][%i]needed %i found %i)",curQst->m_wVarTYPE,curQst->m_wVarNO,curQst->nValue,tempValue);
	...
	if(GServer->ServerDebug)
	{
        if(retVal == 0)
        {
            Log(MSG_DEBUG,"QSDCON 001 returned FAIL");
        }
        else
        {
            Log(MSG_DEBUG,"QSDCON 001 returned SUCCESS");
        }
	}
...
QUESTCOND(010)
{
	...
	{
	    if(GServer->ServerDebug)
            Log(MSG_DEBUG,"Percent chance SUCCESS %i < %i", thischance,data->btHighPcnt);
	    //if( client->questdebug )
        //    server->SendPM(client, "Percent chance SUCCESS %i < %i", thischance,data->btHighPcnt);
	    return QUEST_SUCCESS;
	}
	if(GServer->ServerDebug)
        Log(MSG_DEBUG,"Percent chance FAILURE %i > %i", thischance,data->btHighPcnt);
	//if( client->questdebug )
...
QUESTCOND(011)
{
  ...
  */
    ...
    LogDebug("QSDC11: NPC %i, op=%i, value=%i (real=%i)",refNPC,data->btOp,data->iValue,tempval);
