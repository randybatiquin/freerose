...
bool CWorldServer::LoadSTBData( )
{
    Log( MSG_LOAD, "STB Data" );
        ...
        STBStoreData( "3DData\\STB\\LIST_MOUNT.STB", &STB_ITEM[14] );
        ...
        STBStoreDataChar("3DData\\STB\\LIST_ZONE.STB", &ZoneData);
        STBStoreData("3DData\\STB\\LIST_CLASS.STB", &ListClass);
    ...
    Log( MSG_LOAD, "STL Data" );
        ...
        STLStoreData( "3DData\\STB\\LIST_MOUNT_S.STL",20);
...
bool CWorldServer::LoadLTB( )
{
    ...
    for (unsigned int k=0;k<MyLTB.record.size();k++)
    {
        ...
        GServer->Ltbstring.push_back(tempLTB);
    }
    ...
    for (unsigned int k=0;k<MyLTBQSD.record.size();k++)
    {
        ...
        GServer->LtbstringQSD.push_back(tempLTB);
    }
...
bool CWorldServer::InitDefaultValues()
{
    ...
    MountList.Index = new CMountData*[STB_ITEM[14].rowcount];
    MountList.max=STB_ITEM[14].rowcount;
    ...
    // Observe: Mount null init
    CMountData* nullmount = new CMountData;
    nullmount->id       = 0;
    nullmount->type     = 0;
    nullmount->mspeed   = 0;
    nullmount->wspeed   = 0;
    nullmount->duration = 0;
    nullmount->weight   = 0;
    MountList.nullmount = nullmount;
    for(UINT i=0;i<MountList.max;i++)
    {
        MountList.Index[i] = nullmount;
    }
    ...
    //Arena
    CCListArena* nullarena = new CCListArena;
    nullarena->id = 0;
    nullarena->entrance = 0;
    nullarena->maxlevel = 0;
    nullarena->maxteam= 0;
    nullarena->minlevel = 0;
    nullarena->minteam = 0;
    ArenaList.nullarena = nullarena;
    for(UINT i=0;i<ArenaList.max;i++)
    {
        ArenaList.Index[i] = nullarena;
    }
...
bool CWorldServer::LoadQuestItemData( )
{
    ...
    for (UINT i = 0; i<STB_ITEM[12].rowcount; i++)
    {
        ...
        QuestItemData.push_back(newQuestItem);
    }
...
bool CWorldServer::LoadSkillData( )
{
    ...
    for (unsigned int i = 0; i<STB_SKILL.rowcount;i++)
    {
        ...
        */
        ...
        //PY trouble is though.... These STB columns are still used for Bonfire and other summon healing skills so now they don't work <.<
        //I'm gonna need to get jiggy with it to fix this cock up
        //Adding some stuff to the CSkills class....

        newskill->bonBuff[0] = STB_SKILL.rows[i][21];      // Stat
        newskill->bonValue1[0] = STB_SKILL.rows[i][22];    // Int Value
        newskill->bonValue2[0] = STB_SKILL.rows[i][23];    // % Value
        newskill->bonBuff[1] = STB_SKILL.rows[i][24];      // Stat
        newskill->bonValue1[1] = STB_SKILL.rows[i][25];    // Int Value
        newskill->bonValue2[1] = STB_SKILL.rows[i][26];    // % Value
       //PY new stuff end
        ...
        // start Status Effect - Terr0risT
        newskill->value3[0] = STB_SKILL.rows[i][92];    // Status Effect
        // end Status Effect - Terr0risT
        ...
        // start Status Effect - Terr0risT
        newskill->value3[1] = STB_SKILL.rows[i][96];    // Status Effect
        // end Status Effect - Terr0risT
        ...
        bool foundmaxlevel = false;
        int index = i;
        while (foundmaxlevel == false)
        {
            if (index + 1 < STB_SKILL.rowcount)
            {
                if (STB_SKILL.rows[index][1] != STB_SKILL.rows[index + 1][1])
                {
                    newskill->maxlevel = STB_SKILL.rows[index][2];
                    foundmaxlevel = true;
                }
            }
            else
...
bool CWorldServer::LoadIfoObjects2( )
{
	Log( MSG_LOAD, "Info QSD Triggers" );
	MYSQL_ROW row;

    MYSQL_RES *result=NULL;
        result = DB->QStore("SELECT id, Objecttype, Virtualtype, map, x, y, questname, luaname , eventid FROM list_objects ORDER BY id ASC");
	...
	while( row = mysql_fetch_row(result) )
    {
		CIfoObject* thisobject = new (nothrow) CIfoObject;
        if(thisobject==NULL)
        {
            Log(MSG_ERROR, "Error allocing memory       " );
            DB->QFree( );
		...
		thisobject->id = atoi(row[0]);
		thisobject->Objecttype = atoi(row[1]);
		thisobject->Virtualtype = atoi(row[2]);
		thisobject->map = atoi(row[3]);
		thisobject->dest.x = (float)atof(row[4]);
		thisobject->dest.y = (float)atof(row[5]);
        strcpy(thisobject->questtrigger, row[6]);
        strcpy(thisobject->luaname, row[7]);
		thisobject->eventid = atoi(row[8]);


		IfoObjectList.push_back( thisobject );
	}
...
bool CWorldServer::LoadUpgrade( )
{
    ...
    for (int k=0;k<31;k++)//NB_REF_RULES
    {
        refine_grade[k]=0;
    }
    ...
    while(!feof(fh))
    {
        ...
        if(id>=31||id<1)//NB_REF_RULES
        {
            Log(MSG_WARNING,"Incorrect refine rules ID: %i, must be <%u, change NB_REF_RULES",id,31);
            continue;
        }
...
bool CWorldServer::LoadMountItem( )
{
    Log(MSG_LOAD,"Mount Data STB");
    for(unsigned int i=0; i<STB_ITEM[14].rowcount; i++)
    {
        CMountData* newmount = new (nothrow) CMountData;
        if(newmount==NULL)
        {
            Log(MSG_WARNING, "Error allocing memory: mount\n");
            return false;
        }
        newmount->id        = i;
        newmount->type      = STB_ITEM[14].rows[i][4];
        newmount->weight    = STB_ITEM[14].rows[i][7];
        newmount->mspeed    = STB_ITEM[14].rows[i][28];
        newmount->wspeed    = STB_ITEM[14].rows[i][29];
        newmount->duration  = STB_ITEM[14].rows[i][32];
        newmount->STLId     = STB_ITEM[14].rows[i][33];
        MountList.Index[newmount->id] = newmount;
    }
    Log(MSG_LOAD,"Mount data loaded");
    return true;
}
...
bool CWorldServer::LoadSellData( )
{
    ...
    for(unsigned int i=0;i<STB_SELL.rowcount;i++)
    {
        ...
        for(unsigned int j=6;j<STB_SELL.fieldcount;j++)
        {
            if (j>=56)
            {
                Log(MSG_WARNING,"Loading SELL DATA, Too much Items for %i! %i>56",i,STB_SELL.fieldcount);
                break;
            }

            newsell->item[j-6] = STB_SELL.rows[i][j];
        }
...
bool CWorldServer::LoadGameArenaData( )
{
    Log( MSG_LOAD, "GameArena - STB        " );
    for(unsigned int i=0;i<ListArena.rowcount;i++)
    {

        CCListArena* newarena = new (nothrow) CCListArena;
        if(newarena==NULL)
        {
            Log(MSG_WARNING, "\nError Allocing memory: arena" );
            return false;
        }

        newarena->id = i;
        newarena->minlevel = ListArena.rows[i][1];
        newarena->maxlevel = ListArena.rows[i][2];
        newarena->minteam = ListArena.rows[i][3];
        newarena->maxteam = ListArena.rows[i][4];
        newarena->next = ListArena.rows[i][6];
        newarena->prev = ListArena.rows[i][7];
        newarena->entrance = ListArena.rows[i][9];

        if(newarena->minlevel == 0)
            continue;
        ArenaList.Index[newarena->id] = newarena;
    }
    Log( MSG_LOAD, "Arena Data loaded" );
    return true;
}
...
bool CWorldServer::LoadZoneData( )
{
    ...
    for(unsigned int i=0;i<ZoneData.rowcount;i++)
    {
        ...
        */
        ...
        //if (newzone->allowpvp ==1&&newzone->allowpvp <=2) // - Terr0risT - original value
        if (newzone->allowpvp ==1)
        {
            newzone->allowpvp = 1;
        }
        else
...
bool CWorldServer::LoadItemStats( )
{
    ...
    for(unsigned int i=0;i<STB_ITEM[10].rowcount;i++)
    {
        ...
        tempstats->stat[0] = STB_ITEM[10].rows[i][16];
        tempstats->value[0] = STB_ITEM[10].rows[i][17];
        tempstats->stat[1] = STB_ITEM[10].rows[i][18];
        tempstats->value[1] = STB_ITEM[10].rows[i][19];
        StatsList.push_back(tempstats);
    }
...
bool CWorldServer::LoadGrades( )
{
    ...
    for(unsigned int i=0;i<ListGrade.rowcount;i++)
    {
        ...
        tempGrade->atk_addbonus=ListGrade.rows[i][0];
        tempGrade->atk_percent=ListGrade.rows[i][1];
        tempGrade->acc_addbonus=ListGrade.rows[i][2];
        tempGrade->acc_percent=ListGrade.rows[i][3];
        tempGrade->def_addbonus=ListGrade.rows[i][4];
        tempGrade->def_percent=ListGrade.rows[i][5];
        tempGrade->mdef_addbonus=ListGrade.rows[i][6];
        tempGrade->mdef_percent=ListGrade.rows[i][7];
        tempGrade->dodge_addbonus=ListGrade.rows[i][8];
        tempGrade->dodge_percent=ListGrade.rows[i][9];

        GradeList.push_back(tempGrade);
    }
...
bool CWorldServer::LoadArenaSpawns()
{
    Log( MSG_LOAD, "Loading Arena Spawn zones" );
    MYSQL_ROW row;
    MYSQL_RES *result = DB->QStore("SELECT id,map,npc,type,x,y FROM list_arenaspawnzones");
    ...
    while( row = mysql_fetch_row(result) )
    {
        CCArenaSpawn* thisspawn = new (nothrow) CCArenaSpawn;
        if(thisspawn == NULL)
        {
            Log(MSG_ERROR, "Error allocing memory       " );
            DB->QFree( );
        ...
        thisspawn->id = atoi(row[0]);
        thisspawn->map =  atoi(row[1]);
        thisspawn->npc =  atoi(row[2]);
        thisspawn->type = atoi(row[3]);
        thisspawn->position.x = (float)atof(row[4]);
        thisspawn->position.y = (float)atof(row[5]);
        ArenaSpawnList.push_back(thisspawn);
    }
    DB->QFree( );
...
bool CWorldServer::LoadWayPoints()
{
    ...
    for(int i=0;i<100;i++)
    {
        for(int j=0;j<100;j++)
        {
            GServer->WPList[i][j].X = 0;
            GServer->WPList[i][j].Y = 0;
    ...
    while( row = mysql_fetch_row(result) )
    {
        ...
        GServer->WPList[thismap][thisindex].X = atof(row[2]);
        GServer->WPList[thismap][thisindex].Y = atof(row[3]);
...
bool CWorldServer::LoadConfig( )
{
    ...
    MYSQL_RES *result = DB->QStore("SELECT exp_rate, drop_rate, zuly_rate, blue_chance, stat_chance, slot_chance, \
        ...
        visualdelay, worlddelay, fairymode, fairystay, fairywait, fairytestmode, osrosever, testgrid,\
        monmax, massexport,uwnbplayers,uwside,pc_drop_zuly,drop_rev,advertise_active, advertise_time, item_drop_rate FROM list_config");
    ...
    while( row = mysql_fetch_row(result) )
    {
       ...
       GServer->Config.monmax = atoi(row[31]);
       GServer->Config.massexport = atoi(row[32]);

	   GServer->Config.AdvertiseActive = atoi(row[37]);
       GServer->Config.AdvertiseClock = atoi(row[38]);
       GServer->Config.lastAd = 0;

       GServer->Config.ItemDropRate = atoi(row[39]);

       //LMA: Union Wars.
       UWNbPlayers=atoi(row[33]);
       ObjVar[1113][2]=atoi(row[34]);

        //LMA: % of getting zuly in drop.
       GServer->Config.pc_drop_zuly=atoi(row[35]);
       if(GServer->Config.pc_drop_zuly<=0||GServer->Config.pc_drop_zuly>=100)
       {
           GServer->Config.pc_drop_zuly=30;
           Log(MSG_WARNING,"Wrong value for percentage zuly drop %i, changed to 30",atoi(row[35]),GServer->Config.pc_drop_zuly);
       }
