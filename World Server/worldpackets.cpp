...
void CWorldServer::pakPlayer( CPlayer *thisclient )
{
    ...
    ADDDWORD    ( pak, thisclient->Position->Map );		         // Map
    ADDFLOAT   ( pak, thisclient->Position->current.x*100 );	 // Pos X
    ADDFLOAT   ( pak, thisclient->Position->current.y*100 );	 // Pos Y
    ADDDWORD    ( pak, 0x00 ); // Pos Z ??
    ADDDWORD   ( pak, thisclient->CharInfo->Face );			     // Face
    ADDDWORD   ( pak, thisclient->CharInfo->Hair );	             // Hair
    if(thisclient->items[142].itemnum != 0)//CAP
    {
		ADDWORD    ( pak, thisclient->items[142].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[142]));
    } else {
		ADDWORD    ( pak, thisclient->items[2].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[2]));
    }
    if(thisclient->items[143].itemnum != 0)//BODY
    {
		ADDWORD    ( pak, thisclient->items[143].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[143]));
    } else {
		ADDWORD    ( pak, thisclient->items[3].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[3]));
    }
    if(thisclient->items[144].itemnum != 0)//GLOVES
    {
		ADDWORD    ( pak, thisclient->items[144].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[144]));
    } else {
		ADDWORD    ( pak, thisclient->items[5].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[5]));
    }
    if(thisclient->items[145].itemnum != 0)//Shoes
    {
		ADDWORD    ( pak, thisclient->items[145].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[145]));
    } else {
		ADDWORD    ( pak, thisclient->items[6].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[6]));
    }
    if(thisclient->items[146].itemnum != 0)//FACE ITEM
    {
		ADDWORD    ( pak, thisclient->items[146].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[146]));
    } else {
		ADDWORD    ( pak, thisclient->items[1].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[1]));
    }
    if(thisclient->items[147].itemnum != 0)//BACK ITEM
    {
		ADDWORD    ( pak, thisclient->items[147].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[147]));
    } else {
		ADDWORD    ( pak, thisclient->items[4].itemnum );
		ADDWORD    ( pak, BuildItemRefine( thisclient->items[4]));
    }
    ...
    ADDBYTE    ( pak, thisclient->CharInfo->HairColor );
    ADDBYTE    ( pak, thisclient->CharInfo->Face );
    ADDBYTE    ( pak, thisclient->CharInfo->Hair );
	ADDDWORD    ( pak, thisclient->CharInfo->Job );
    ADDBYTE( pak, 0x00 );
	...
	ADDDWORD    ( pak, thisclient->Stats->HP );                   // Current HP
	ADDDWORD    ( pak, thisclient->Stats->MP );                   // Current MP
	...
	ADDWORD( pak, thisclient->items[136].sp_value);//Fuel
    for(int i=0; i<3; i++) ADDWORD( pak, 0x00 );
	...
	ADDWORD(pak, thisclient->CharInfo->Honor); //Honor Points
	ADDWORD(pak, thisclient->CharInfo->Valor); //Valor Points
	ADDDWORD(pak, 0x646f); // null ?? luckyspin ? itemmall points??
	ADDWORD(pak, thisclient->CharInfo->Premium); //Premium Points
	for(int i=0; i<165; i++) ADDWORD(pak, 0x00);
	for(int i=0; i<48; i++)ADDWORD( pak, thisclient->quickbar[i] );
	ADDDWORD ( pak, thisclient->CharInfo->charid );	            // CharID
	for(int i=0; i<80;i++) ADDBYTE( pak, 0x00 );
...
bool CWorldServer::pakDoIdentify( CPlayer *thisclient, CPacket *P )
{
    ...
    pakCostumeEquipment(thisclient);
    pakQuestData(thisclient);
    pak724(thisclient);
    pak855(thisclient);
    pakServerTime(thisclient);

	thisclient->SetStats( );
	return true;
}

void CWorldServer::pakServerTime(CPlayer* thisclient)
{
	time_t rawtime;
	struct tm* timeinfo;
	timeinfo  = localtime( &rawtime );

	BEGINPACKET(pak, 0x826);

	ADDDWORD(pak, 0x00000000);// null
	ADDDWORD(pak, 0x00000000);// null
	ADDDWORD(pak, GetServerTime());
	ADDDWORD(pak, 0x00000000);// null
	ADDDWORD(pak, timeinfo->tm_year);
	ADDDWORD(pak, timeinfo->tm_mon);
	ADDDWORD(pak, timeinfo->tm_mday);
	ADDDWORD(pak, timeinfo->tm_hour);
	ADDDWORD(pak, timeinfo->tm_min);
	ADDDWORD(pak, timeinfo->tm_sec);
	ADDDWORD(pak, timeinfo->tm_isdst);

	thisclient->client->SendPacket( &pak );
}

void CWorldServer::pakCostumeEquipment( CPlayer *thisclient )
{
	BEGINPACKET(pak, 0x725);
	for(UINT i = 140; i < MAX_INVENTORY; i++)
	{
		ADDDWORD(pak, BuildItemHead(thisclient->items[i]));
		ADDDWORD(pak, BuildItemData(thisclient->items[i]));
		ADDDWORD(pak, 0x00000000);
		ADDWORD(pak, 0x0000);
	}
...
void CWorldServer::pak855( CPlayer *thisclient )
{
	BEGINPACKET(pak, 0x855);
		ADDBYTE(pak, 0x01);
		ADDQWORD(pak, 0);
	thisclient->client->SendPacket( &pak );
}

void CWorldServer::pak724( CPlayer *thisclient )
{
	BEGINPACKET(pak, 0x724);
		for(int i=0;i < 420; i++) ADDBYTE(pak, 0x00);
	thisclient->client->SendPacket( &pak );
}
...
bool CWorldServer::pakDoID( CPlayer* thisclient, CPacket* P )
{
    ...
    if( thisclient->Party->party )
    {
        ...
        ADDDWORD    ( pak, thisclient->GetMaxHP( ) );
        ADDDWORD    ( pak, thisclient->Stats->HP );
        ADDDWORD   ( pak, GServer->BuildBuffs( thisclient ));
        ADDDWORD   ( pak, 0x00000000 );
        ADDDWORD   ( pak, 0x00000000 );
        ADDWORD   ( pak, 0x00000000 );
        ADDWORD    ( pak, 0x1388 );
        thisclient->Party->party->SendToMembers( &pak, thisclient );

        RESETPACKET(pak, 0x7f5);
        ADDDWORD(pak, thisclient->Stats->HP);
        ADDDWORD(pak, thisclient->Stats->MP);
        ADDDWORD(pak, 0x00000003);
    ...
    ADDWORD    ( pak, 100 );  // World Rate
    ADDBYTE    ( pak, 0x70 );
    ADDBYTE    ( pak, 0x69 );
    ADDBYTE    ( pak, 0x68 );
    ADDBYTE    ( pak, 0x67 );
    ADDWORD    ( pak, 100 );  // Town rate
    ADDBYTE    ( pak, 0x50 );  // misc rate
    ADDBYTE    ( pak, 0xcd );  //1
    ADDBYTE    ( pak, 0x2d );  //2
    ADDBYTE    ( pak, 0x2d );  //3
    ADDBYTE    ( pak, 0x2d );  //4
    ...
    if(map->allowpvp != 1)
    {
        ...
        ADDWORD(pak, 0x0000);
        ADDDWORD ( pak, 0x00000000 ); // friendly
    }
    else
    {
        //PVP maps - Terr0risT
        if(map->id == 5 || map->id == 6)
        {
            ADDDWORD( pak, 0x00000064 ); // hostile
        }
        else
        {
            ADDWORD(pak, 0x0001);
    ...
    Send_pakGroupMemberList(thisclient->Groupga->Groupga, 1);// List of all members

    //LMA: Are we in overweight?
    BYTE weight = 0;
    if(thisclient->GetMaxWeight() > 0)
    {
    	weight = thisclient->GetCurrentWeight() / thisclient->GetMaxWeight() * 100;
    }
	...
	if(thisclient->Status->Stance=MOUNTED)
    thisclient->Stats->Move_Speed = thisclient->GetMoveSpeed( );
	{
        RESETPACKET  ( pak, 0x858 );
        ADDWORD     ( pak, thisclient->clientid );
        ADDDWORD    ( pak, 0 );
        ADDBYTE     ( pak, 0 );
        ADDDWORD    ( pak, thisclient->Stats->Base_Speed );
...
{
    if( thisclient->Shop->open || (!thisclient->Ride->Drive && thisclient->Ride->Ride) || !thisclient->Status->CanMove || thisclient->Status->Faint != 0xff || thisclient->Status->Sleep != 0xff)
        return true;
...
bool CWorldServer::pakSpawnNPC( CPlayer* thisclient, CNPC* thisnpc )
{
    ...
    {
       ADDWORD( pak, GServer->Config.Cfmode );
    } else {
    	if(thisnpc->event != 0)
    	{
...
{
   if(thisclient->IsDead() || thisclient->Shop->open) return true;

	BYTE stancenum = GETBYTE((*P),0);
	...
	switch(stancenum)
	{
		case 0:
		{
			...
			else if(thisclient->Status->Stance != DRIVING) // Running
			{
				stancenum = RUNNING;
				thisclient->Status->Stance = RUNNING;
			}
		...
		case 1:
		{
			if(thisclient->Status->Stance != DRIVING && thisclient->Status->Stance > 1)
			{
				thisclient->Status->Stance = MOVING;
			} else if(thisclient->Status->Stance < DRIVING)// < 4
			{
				thisclient->Status->Stance = RUNNING;
			}
		}break;
...
bool CWorldServer::pakChangeEquip( CPlayer* thisclient, CPacket* P)
{
	...
	if(!CheckInventorySlot(thisclient, srcslot) || !CheckInventorySlot(thisclient, destslot)) return false;
	...
	if(switched)
	{
		if(destslot > 0 && destslot < 4)
		{
			check = destslot + 140;
		} else if(destslot > 4 && destslot < 7) {
			check = destslot +139;
		}
		if(thisclient->items[check].itemnum == 0 && thisclient->items[check].itemtype == 0)
			SetVisibleEquipment(thisclient, destslot);
	} else {
		if(srcslot > 0 && srcslot < 4)
		{
			check = srcslot + 140;
		} else if(srcslot > 4 && srcslot < 7) {
			check = srcslot +139;
		}
		if(thisclient->items[check].itemnum == 0 && thisclient->items[check].itemtype == 0)
			SetVisibleEquipment(thisclient, srcslot);
	}
...
bool CWorldServer::pakChangeCostume( CPlayer* thisclient, CPacket* P)
{
	...
	thisclient->UpdateCostume( srcslot,destslot);
	if(switched)
		SetVisibleEquipment(thisclient, destslot);
	else
		SetVisibleEquipment(thisclient, srcslot);
...
bool CWorldServer::pakChangeCart( CPlayer* thisclient, CPacket* P )
{
	...
	WORD destslot = GETWORD((*P), 0) + 135;
	WORD srcslot = GETWORD((*P), 2);
    ...
    if(thisclient->Status->Stance==DRIVING&&srcslot!=139&&destslot!=139)
        return true;
	...
	if(!CheckInventorySlot( thisclient, srcslot ))
	   return false;
	if( srcslot == 0 ) srcslot=thisclient->GetNewItemSlot( thisclient->items[destslot] );
	if( srcslot == 0xffff ) return true;

	//LMA: we need to check if all items have the same PatType...
    //LMA: test only if equip.
	else if (destslot>=135&&destslot<140&&thisclient->items[srcslot].itemnum>0)
	{
        for (int k=135;k<140;k++)
        {
            if (k == 135 && CheckPatList(thisclient) == 1)
                continue;
            if(thisclient->items[k].itemnum==0)
                continue;
            if (PatList.Index[thisclient->items[srcslot].itemnum]->parttype != PatList.Index[thisclient->items[k].itemnum]->parttype)
            {
	...
	CItem tmpitm = thisclient->items[destslot];
    ...
    thisclient->UpdateInventory( destslot, srcslot );
	...
	//ADDWORD    ( pak, BuildItemRefine( thisclient->items[destslott] ) );
...
{
    if(!thisclient->Status->CanMove || thisclient->Status->Sleep != 0xff || thisclient->Status->Faint != 0xff)
        return true;
    if(thisclient->Shop->open)
    {
       GServer->SendPM(thisclient,"Cant attack while vending");
       return true;
    }
    //Log(MSG_HACK, "%s STARTS TO ATTACK",thisclient->CharInfo->charname);
...
bool CWorldServer::pakGate( CPlayer* thisclient, CPacket* P )
{
	...
	for (int k=0;k<6;k++)
	{
	    thisgate = GetTeleGateByID( GateID,k);
            ...
            thisclient->Status->Stance = RUNNING;
            thisclient->Stats->Move_Speed = thisclient->GetMoveSpeed( );
            Log(MSG_DEBUG,"%s Dismounted", thisclient->CharInfo->charname);
        }
	    ...
	    if(thisclient->Position->Map != thisgate->srcMap ||distance(thisclient->Position->current, thisgate->src) > 200)
	    {
            //hack or wrong gate?
            lastgate=thisgate;
            continue;
...
bool CWorldServer::pakNormalChat( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
		...
		Log( MSG_CHAT, "%s: %s",thisclient->CharInfo->charname,P->Buffer );
	}
...
bool CWorldServer::pakWhisper ( CPlayer* thisclient, CPacket* P )
{
	...
	if(otherclient!=NULL)
	{
	   ...
	   Log( MSG_WHISPER, "%s: %s",thisclient->CharInfo->charname,P->Buffer );
	   return true;
	}
...
bool CWorldServer::pakChatTrade ( CPlayer* thisclient, CPacket* P )
{
	...
	ADDBYTE    ( pak, 0 );
	SendToMap  ( &pak, thisclient->Position->Map );
    Log( MSG_TRADE, "%s: %s",thisclient->CharInfo->charname,P->Buffer );
...
bool CWorldServer::pakLuckySpin( CPlayer* thisclient, CPacket* P )
{
    ...
    GServer->DB->QFree( );
        ...
        RESETPACKET( pak, 0x7ef );
        ADDBYTE    ( pak, 0x01 );                                   // Action
        ADDDWORD   ( pak, thisclient->CharInfo->LuckyShuffles );                           // Shuffles Left
...
// Observe: Lucky spin item list
bool CWorldServer::pakLuckySpinList( CPlayer* thisclient, CPacket* P )
{   //                                              0       1         2
	MYSQL_RES *result = GServer->DB->QStore("SELECT itemid, itemtype, slot FROM luckyspin WHERE active='1'");
	if(result==NULL) return false;


	BEGINPACKET(pak, 0x7ee);
	ADDBYTE (pak, 0x00 );
	ADDDWORD (pak, mysql_num_rows(result));
	...
	{
        int count = mysql_num_rows(result);
        int ItemNumber = (atoi(row[1])*1000000) + atoi(row[0]);
		if(x < count)
		{
			ADDDWORD (pak, atoi(row[2]));
			ADDDWORD (pak, ItemNumber);
			ADDDWORD (pak, 1);
    ...
    RESETPACKET(pak, 0x7ee);
    ADDBYTE(pak, 0x01); // End
    ADDDWORD(pak, 0);
    thisclient->client->SendPacket( &pak);
...
{
    BYTE spins = GETBYTE((*P), 0);

    if(thisclient->CharInfo->Itemmall < spins*12 )
    {
    ...
    {
        thisclient->CharInfo->Itemmall   -= spins*12;
        thisclient->CharInfo->LuckySpins += spins;
        SaveItemMallPoints(thisclient);
        BEGINPACKET ( pak, 0x7f0 );
        ADDBYTE     ( pak, 1 );
        ADDDWORD    ( pak, thisclient->CharInfo->LuckySpins );
        thisclient->client->SendPacket( &pak );

        RESETPACKET ( pak, 0x7c9 );
        ADDBYTE     ( pak, 0 );
...
bool CWorldServer::pakGameListReq( CPlayer* thisclient, CPacket* P)
{
    int amount = 0;

    BEGINPACKET( pak, 0x830 );
    ADDDWORD(pak, 0);

    for(int i=0;i<ArenaList.max;i++)
    {
        if(ArenaList.Index[i]->prev == 0 && ArenaList.Index[i]->next != 0)
        {
            amount++;
            ADDDWORD(pak, i);
            if(thisclient->Stats->Level < ArenaList.Index[i]->minlevel)
            {
                ADDWORD(pak, 1);
            } else {
                ADDWORD(pak, 0);
...
// toxin: GameArena
bool CWorldServer::pakQueueJoin( CPlayer* thisclient, CPacket* P )
{
    DWORD id = GETDWORD((*P), 0);
    GAGroup* thisgroup = thisclient->Groupga->Groupga;
    if(thisgroup == NULL) return true;
    BEGINPACKET     ( pak, 0x831 ); //ok we added you
    ADDBYTE         ( pak, 0x00);
    ADDDWORD        ( pak, id);
    thisgroup->SendToMembers( &pak );

    RESETPACKET     ( pak, 0x836    ); //queue infos
    ADDDWORD        ( pak, id       ); // stb id
    ADDDWORD        ( pak, 1        );
    ADDDWORD        ( pak, 1        );//how much groups are in queue (total)
    ADDDWORD        ( pak, 0        ); //how much active games
    ADDDWORD        ( pak, 0        ); //how much Waiting
    thisgroup->SendToMembers( &pak );

    RESETPACKET     ( pak, 0x833 ); //ok we removed you
    ADDBYTE         ( pak, 1    );
    ADDDWORD        ( pak, id);if(thisclient->Groupga->Groupga == NULL)
    thisgroup->SendToMembers( &pak );

    RESETPACKET     ( pak, 0x728 ); //ok we removed you
    ADDWORD         ( pak, 0056    );
    ADDDWORD         ( pak, 00011170);
    thisgroup->SendToMembers( &pak );
    int map;
    switch(id)
    {
    ...
    GServer->SeaOfDawn(thisgroup,map);

    return true;
}
// toxin: GameArena
bool CWorldServer::pakQueueLeave( CPlayer* thisclient, CPacket* P )
{
    DWORD id = GETDWORD((*P), 0);
    ...
    BEGINPACKET     ( pak, 0x832 ); //ok we removed you
    ADDBYTE         ( pak, 0x00);
    ADDDWORD        ( pak, id);
    thisgroup->SendToMembers( &pak );

    RESETPACKET ( pak, 0x850 );
    ADDBYTE     ( pak, 1 );
    thisgroup->SendToMembers(&pak);
...
// toxin: GameArena
bool CWorldServer::pakJoinArena( CPlayer* thisclient, CPacket* P )
{
...
// toxin: GameArena
bool CWorldServer::pakGroupChat( CPlayer* thisclient, CPacket* P )
{
    GAGroup* thisgroup = thisclient->Groupga->Groupga;
    if(thisgroup == NULL)
    {
    ...
    //OUT 0851: 01 a8 6f 02 00
}
bool CWorldServer::pakSendGroupID( CPlayer* thisclient, CPacket* P )
{
    WORD id = GETWORD((*P), 0);
    GAGroup* thisgroup = thisclient->Groupga->Groupga;
    if(thisgroup == NULL)
...
// Observe: Buy spins with coins!
bool CWorldServer::pakLuckySpinCoins ( CPlayer* thisclient, CPacket* P )
    {
    // Packet 7f3
    /*
    IN 07f3: 4c 00 00 00 01 00 00 00
    IN 07f3: 4c 00 00 00 05 00 00 00
    OUT 0718: 01 4c 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
    OUT 07f3: 01 01 00 00 00
    */
    }
...
bool CWorldServer::pakSpinWheel ( CPlayer* thisclient, CPacket* P )
  {
        ...
        if(thisclient->CharInfo->LuckySpins < 1){
            ADDBYTE  ( pak, 2 );
            ADDDWORD ( pak, 0 );
            ADDDWORD ( pak, 0 );
            thisclient->client->SendPacket( &pak );
            ...
            GServer->DB->QExecute("INSERT INTO mileage (`owner`,`itemnum`,`itemtype`,`count`,`slotnum`) VALUES ('%i','%i','%i','1','%i')",thisclient->Session->userid,itemID,itemType,slotNum);
            thisclient->savedata( );

            luckychoice[0] = 1; //original value is 0
        }
        return true;
    }

// Observe: Redeem item a day
bool CWorldServer::pakPremiumRedeem ( CPlayer* thisclient, CPacket* P )
{
    ...
    for (int i=0; i<numrow; i++)
    {
        ...
        if (thisclient->GetNewItemSlot(thisclient->itemmallitems[slot]) != 0xffff)
        GServer->DB->QExecute("DELETE FROM mileage WHERE owner='%i' AND slotnum='%i'", thisclient->Session->userid,slot);
    }
...
// Observe: Mounts
bool CWorldServer::pakMount (CPlayer* thisclient, CPacket* P)
{
                if (thisclient->Ride->Ride)
                {
                    GServer->SendPM(thisclient,"Cant mount while riding");
                    return true;
                }
                if(thisclient->Shop->open)
                {
                    GServer->SendPM(thisclient,"Cant mount while vending");
                    return true;
                }
    ...
    switch(thisclient->Status->Stance)
    {
        case WALKING:
        case RUNNING:
        case MOVING:
        ...
        default:
        {
            Log(MSG_DEBUG,"Mounting wasnt successfull for player: %s", thisclient->CharInfo->charname);
        }
        break;
    }
...
// Observe Buy Itemmall Item
bool CWorldServer::pakItemmallBuy ( CPlayer* thisclient, CPacket* P )
{
    byte Action   = GETBYTE((*P), 0);
    byte button   = GETBYTE((*P), 1);
    DWORD Slot    = GETDWORD((*P), 2);
    byte itemType = GETBYTE((*P), 6);
    WORD itemID   = GETWORD((*P), 10);
    byte Amount   = GETBYTE((*P), 14);
if (GetMileageFreeSlotOnline(thisclient) != -1)
{
    switch(Action)
    {
        case 1: // Send request to buy
        {
            BEGINPACKET ( pak, 0x727 );
            ADDBYTE     ( pak, 4 );
            ADDBYTE     ( pak, 1 );
            ADDDWORD    ( pak, Slot );
            ADDDWORD    ( pak, itemType );
            ADDDWORD    ( pak, itemID );
            ADDBYTE     ( pak, Amount );
            ADDDWORD    ( pak, 0 );
            ADDBYTE     ( pak, 16 );
            ADDWORD    ( pak, 0 );
            ADDDWORD    ( pak, 0x002b01b5 );
            for(byte i=0; i<14; i++){
                ADDBYTE ( pak, 0 );
            }
            ADDWORD    ( pak, 0x1388 );
        ...
        case 3: // Bring Item
        {
            if(thisclient->CharInfo->Itemmall < thisclient->ItemMallItemsPrice[Slot])
            {
            BEGINPACKET ( pak, 0x727 );
            ADDDWORD    ( pak, 10 );
            ADDDWORD    ( pak, 0x3d10014e );
            ADDDWORD    ( pak, 0x83c94904 );
            ADDBYTE     ( pak, 0 );
            ADDDWORD    ( pak, 0x0000de49 );
            ADDBYTE     ( pak, 0 );
            ADDDWORD    ( pak, 0x0000002a );
            ADDDWORD    ( pak, 0x000031d0 );
            ADDDWORD    ( pak, 0x000031d0 );
            ADDDWORD    ( pak, 0 );
            ADDDWORD    ( pak, 0x000010bd );
            ADDDWORD    ( pak, 0x000010d1 );
            ADDDWORD    ( pak, 0x0000008e );
            ADDWORD     ( pak, 0x000f );
            ADDDWORD    ( pak, 0x0070ad2f );
            ADDWORD     ( pak, 0 );
            ADDDWORD    ( pak, 0x61736150 );
            ADDDWORD    ( pak, 0x4a796177 );
            ADDDWORD    ( pak, 0xad006569 );
            ADDDWORD    ( pak, 0xae98013a );
            ADDDWORD    ( pak, 0xafa0013a );
            ADDDWORD    ( pak, 0xb0a8013a );
            ADDDWORD    ( pak, 0xb1b0013a );
            ADDDWORD    ( pak, 0xb2b8013a );
            ADDDWORD    ( pak, 0xb3c0013a );
            ADDDWORD    ( pak, 0xb4c8013a );
            ADDDWORD    ( pak, 0xb5d0013a );
            ADDWORD     ( pak, 0 );
            ADDDWORD    ( pak, 0x000000b4 );
            ADDDWORD    ( pak, 0 );
            ADDDWORD    ( pak, 0x000011ed );
            ADDDWORD    ( pak, 4 );
            ADDDWORD    ( pak, 0 );
            ADDWORD     ( pak, 0 );
            ADDDWORD    ( pak, 0x000011ad );
            ADDDWORD    ( pak, 0x00000018 );
            ADDDWORD    ( pak, 0 );
            ADDWORD     ( pak, 0 );
            ADDDWORD    ( pak, 0x000011cd );
            ADDDWORD    ( pak, 0x0000000e)
            ...
            else
            {
                if (button == 1)
                {
                 CItem newitem = CreateItem(itemID, itemType, Amount);
                 int mileageslot = GetMileageFreeSlotOnline(thisclient);
                 thisclient->itemmallitems[mileageslot] = newitem;
                 SaveSlotMall(thisclient, mileageslot);
                 thisclient->CharInfo->Itemmall -= (thisclient->ItemMallItemsPrice[Slot]) * Amount;
                 SaveItemMallPoints(thisclient);
                 ...
                 for (byte i=0; i<144;i++)
                     ADDBYTE ( pak, 0x00 );
                 thisclient->client->SendPacket( &pak );
                 RESETPACKET ( pak, 0x727 );
                 ADDBYTE     ( pak, 7 );
                 for (byte i=0; i<161;i++)
                 ADDBYTE ( pak, 0x00 );
                 thisclient->client->SendPacket( &pak );
        ...
        break;
        case 4: // Cancel clicked
        {
// 04 01 00 00 00 00 0a 00 00 00 c7 00 00 00 01 00 00 00 36 32 32 38 66 64 61 30 35 31 30 65 63 65 61 39 55 50 6e 76 69 73 73 65 13 61 bb 02 00 00 20 00 00 00 00 00 00 00 00 00 00 00 08 ef d0 02 0b 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0f 00 00 00 00 00 00 00 00 00 00 61 d1 00 00 00 00 00 00 00 00 00 00 00 00 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 00 00
//            BEGINPACKET ( pak, 0x727 );
//            ADDBYTE     ( pak, Action );
//            ADDBYTE     ( pak, 1 );
//            ADDDWORD    ( pak, 0 );
...
{
   GServer->SendPM(thisclient,"Mileage Storage Is Full");
}
    return true;
}
...
bool CWorldServer::pakNPCBuy ( CPlayer* thisclient, CPacket* P )
{
    ...
    if(thisnpc->npctype == 2002 || thisnpc->npctype == 2003 || thisnpc->npctype == 3002) // HONOR POINTS NPC
    {
        is_honor = true;
         adding = 1;
    }

    if(thisnpc->npctype >= 2011 && thisnpc->npctype <= 2014) // VALOR POINTS
    {
        is_valor = true;
         adding = 1;
    }

    if(thisnpc->npctype == 1080)  // PREMIUM POINTS
    {
        is_premium = true;
         adding = 1;
    }
	...
	for (int i=0; i<buycount; i++)
    {
        ...
        sellid = maintab + subtab + adding;
        Log(MSG_WARNING,"tabID is %i and sellID %i",tabid,sellid);
        ...
        while (tmpcount>0)
        {
            ...
            if (thisitem.itemtype==0 || thisitem.itemnum==0)
            {
                ...
                GServer->SendPM(thisclient,"Sorry, there is a problem with this NPC, warn admin Terr0risT.");
                return true;
            }
			...
			if(is_valor==true && thisitem.itemtype<10)
                {
                    thisitem.refine = 0;
                    thisitem.socketed = true;
                    thisitem.durability = GServer->STB_ITEM[thisitem.itemtype-1].rows[thisitem.itemnum][29];
                }
			else if(is_honor==true && thisitem.itemtype<10) // Donation System used by FreeROSE. - Terr0risT
                {
                    thisitem.refine = 240;
                    thisitem.socketed = true;
    		...
    		switch(thisitem.itemtype)
    		{
                ...
                case 14:
                {
                    ...
                    //LMA: price tweak.
                    if(!is_clanshop&&!is_union&&!is_honor&&!is_valor&&!is_premium)
                    {
                    ...
                    } else if(is_premium)
                    {
                        ...
                        if (thisclient->CharInfo->Premium<(long int) price)
                        {
                            Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Premium,(long int) price);
                            return true;
                        }

                        temp_price_li = thisclient->CharInfo->Premium;
                        temp_price_li -= (long int) price;

                        if(temp_price_li>thisclient->CharInfo->Premium)
                        {
                            Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Premium,(long int) price);
                            return true;
                        }

                        thisclient->CharInfo->Premium = temp_price_li;
                    } else if (is_valor)
                    {
                        if(thisitem.itemtype < 10 )
                        {
                        ...
                        if (thisclient->CharInfo->Valor<(long int) price)
                        {
                            Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Valor,(long int) price);
                            return true;
                        }

                        temp_price_li = thisclient->CharInfo->Valor;
                        temp_price_li -= (long int) price;

                        if(temp_price_li>thisclient->CharInfo->Valor )
                        {
                            Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Valor ,(long int) price);
                            return true;
                        }

                        thisclient->CharInfo->Valor = temp_price_li;
                    ...
                    } else if( is_honor)
                    {
                        ...
                        if (thisclient->CharInfo->Honor<(long int) price)
                        {
                            Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Honor,(long int) price);
                            return true;
                        }

                        temp_price_li = thisclient->CharInfo->Honor;
                        temp_price_li -= (long int) price;

                        if(temp_price_li>thisclient->CharInfo->Honor )
                        {
                            Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Honor ,(long int) price);
                            return true;
                        }

                        thisclient->CharInfo->Honor = temp_price_li;
                    }
                    else
                    {
                ...
                case 12:
                {
                    ...
                    if(flag)
                    {
                        ...
                        if(!is_clanshop&&!is_union&&!is_honor&&!is_valor&&!is_premium)
                        {
                            if(thisnpc->npctype == 1096) // Mildun in Junon Polis will no longer collect tax.
                            {
                    ...
                    else
                    {
                        ...
                        else if (is_honor)
                        {
                            ...
                            if (thisclient->CharInfo->Honor<(long int) price*count)
                            {
                              Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Honor,(long int) price*count);
                              return true;
                            }
                            ...
                            if(temp_price_li>thisclient->CharInfo->Honor)
                            {
                                Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Honor,(long int) price*count);
                                return true;
                            }
                        ...
                        else if (is_valor)
                        {
                            ...
                            if (thisclient->CharInfo->Valor<(long int) price*count)
                            {
                              Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Valor,(long int) price*count);
                              return true;
                            }
                            ...
                            if(temp_price_li>thisclient->CharInfo->Valor)
                            {
                                Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Valor,(long int) price*count);
                                return true;
                            }
                        ...
                        }else if (is_premium)
                        {
                            ...
                            if (thisclient->CharInfo->Premium<(long int) price*count)
                            {
                              Log(MSG_HACK, "Not enough Union points player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Premium,(long int) price*count);
                              return true;
                            }
                            ...
                            if(temp_price_li>thisclient->CharInfo->Premium)
                            {
                                Log(MSG_HACK, "negative union point hack detected for player %s, have %u, need %u",thisclient->CharInfo->charname,thisclient->CharInfo->Premium,(long int) price*count);
                                return true;
                            }
                ...
                case 13:
                {
                    ...
                    //LMA: price tweak.
                    if(!is_clanshop&&!is_union&&!is_valor&&!is_honor&&!is_premium)
                    {
                        price = (float)round(price*pc_reb);
                    } else {
                        price = (float)round(price);
    ...
    if(is_honor && buycount>0)
    {

    }

    if(is_valor && buycount>0)
    {

    }

    if(is_premium && buycount>0)
    {
...
bool CWorldServer::pakStartSkill ( CPlayer* thisclient, CPacket* P )
{
	...
	if((character->IsPlayer() || character->IsSummon()) && thisskill->skilltype == 19) // Original value is 19
    {
    ...
    if((character->IsMonster() || character->IsSummon()) && thisskill->skilltype == 19) // Original value is 19
    {
    ...
    thisclient->Battle->contatk = true;
    if( isSkillTargetFriendly( thisskill ) && skillnum != 103)
    {
			...
			if (otherclient->Trade->trade_status==6)
            {
				...
				for (unsigned i=0; i<10; i++)
                {
					if(thisclient->Trade->trade_count[i] > 0)
                    {
						...
						ADDWORD( pakt, (unsigned char)thisclient->Trade->trade_itemid[i] );
					...
					if(otherclient->Trade->trade_count[i] > 0)
                    {
                        ...
                        thisclient->items[newslot]=thisitem;
						ADDWORD( pako, (unsigned char)otherclient->Trade->trade_itemid[i] );
...
// Trade item
bool CWorldServer::pakTradeAddItem ( CPlayer* thisclient, CPacket* P )
{
    if(thisclient->Shop->open)
        return true;
...
// Trade zulie
bool CWorldServer::pakTradeAddZulie ( CPlayer* thisclient, CPacket* P )
{
    QWORD zulie = GETQWORD((*P),0);
    	CPlayer* otherclient = GetClientByID( thisclient->Trade->trade_target, thisclient->Position->Map );
	if (otherclient==NULL) return true;
        if (zulie > thisclient->CharInfo->Zulies)
           return true;
        BEGINPACKET( pak, 0x7fb );
		ADDQWORD( pak, zulie );
		thisclient->Trade->trade_count[0x0a] = zulie;
		otherclient->client->SendPacket( &pak );
		return true;
}
...
bool CWorldServer::pakQuestReq( CPlayer* thisclient, CPacket* P )
{
  byte action = GETBYTE((*P),0);
  ...
  {
      SendPM( thisclient, "pakQuestReq Event Trigger [%08x] Action %i", hash, action);
  }
  if(GServer->ServerDebug)
      Log(MSG_DEBUG,"Received 0x730 packet in pakQuestReq");

  //PY Let's start over completely. This is a mess
  switch(action)
  {
      case 1:  // send text. this will probably never happen on incoming packets
        Log(MSG_DEBUG,"Received 0x730 packet with action %i",action);
      break;
      case 2:  //remove quest
        if( thisclient->questdebug )
        {
        ...
        for (i = slot; i < 9; i++) thisclient->quest.quests[i] = thisclient->quest.quests[i+1];         //move them all down 1
        thisclient->quest.quests[9].Clear();                                                            //clear slot 9
        thisclient->SendQuestUpdate();                                                                  //723 packet

        thisclient->Send_pakQuestReply(3, slot, 0, hash);
        return true;
      break;
      case 3:

		Log(MSG_WARNING, "Quest Trigger Start");

        success = thisclient->ExecuteQuestTrigger(hash, slot);

        Log(MSG_DEBUG, "Quest Trigger Returned %i", success);

        thisclient->SendQuestUpdate();
        thisclient->Send_pakQuestReply(success, slot, 0, hash);
        return true;
      break;
      case 4:
        Log(MSG_DEBUG,"Received 0x730 packet with action %i",action);
      ...
      break;
      default:
        Log(MSG_DEBUG,"Received 0x730 packet with unknown action %i",action);
      break;
  }
  return false;
}
bool CWorldServer::pakQuestTrigger( CPlayer* thisclient)
{
    if(thisclient==NULL) return true;
    for(UINT i=0;i<GServer->IfoObjectList.size();i++)
        {
            CIfoObject* thisobject = GServer->IfoObjectList.at(i);
            if(thisclient->Position->Map == thisobject->map)
            {
                GAGroup* thisgroup = thisclient->Groupga->Groupga;
                int distance = 5;
                float resdist = GServer->distance( thisclient->Position->current, thisobject->dest );
                if( resdist < distance )
                {
                    Log(MSG_DEBUG,"He is on the questtrigger %s",thisobject->questtrigger);
                    if(thisobject->id == 598)
                    {
                        BEGINPACKET( pak, 0x7f8 );
                        ADDSTRING    ( pak, "HUD_DawnSea_GateBlocked" );
                        ADDBYTE    ( pak, 0x00 );
                        thisclient->client->SendPacket(&pak);
                    }
                    else
                    {
                        thisclient->ExecuteQuestTrigger(MakeStrHash(thisobject->questtrigger),true);
                    }
                }
            }
        }
    ...
    if(thisclient->IsPlayer())
    {
        for(UINT i=0;i<3; i++)
        {
            if(thisskill->rskill[i] != 0)
            {
                ...
                if(rskill == 0xffff)
                {
                    Log(MSG_DEBUG,"%s:: Necessary skill %u not found to use %u",thisclient->CharInfo->charname,thisskill->rskill[i],skillid);
                    return true;
                }
                else
                {
                    if(thisskill->lskill[i] > thisclient->cskills[rskill].level)
                    {
                        Log(MSG_DEBUG,"%s:: Necessary skill (%u) level incorrect (%u>%u) to use %u",thisclient->CharInfo->charname,thisskill->rskill[i],thisskill->lskill[i],thisclient->cskills[rskill].level,skillid);
                        return true;
                    }
                }
            }
       ...
       case 0: //  cherry berrys
        ...
        case 1: // Food and mana jam
        {
            ...
            if(thisuse->usetype==37)
            {
                ...
                Log(MSG_WARNING, "Current SkillPoints %i added by: %i from the JAM", thisclient->CharInfo->SkillPoints, thisuse->usevalue );
                BEGINPACKET( pak,0x7a3 );
                ADDWORD    ( pak, thisclient->clientid );
                ADDWORD    ( pak, thisuse->itemnum );
                SendToVisible( &pak, thisclient );

                BEGINPACKET( pak2, 0x79e );
                ADDWORD( pak2, thisclient->clientid );
                ADDWORD( pak2, thisclient->Stats->Level );
                ADDDWORD( pak2, thisclient->CharInfo->Exp );
                ADDWORD( pak2, thisclient->CharInfo->StatPoints );
            ...
            thisclient->UsedItem->userate = thisuse->usevalue / 10;
        ...
        case 5: // Summons
        {
            ...
            if(thisnpc==NULL)
            {
                Log( MSG_WARNING,"[%i]NPCDATA NOT Found: %i.",
        ...
        case 17:
        {
            ...
            if( thisclient->items[slot].count <= 0 )
                ClearItem( thisclient->items[slot] );
            thisclient->UpdateInventory(slot,false,0xffff);
            flag=false;
        }
        break;
        case 18:
...
bool  CWorldServer::pakLearnSkill( CPlayer *thisclient, CPacket* P )
{
    bool takeSP;

    WORD skill = GETWORD((*P), 0 );
    int b=1;
...
bool CWorldServer::pakChangeItemMallPoints( CPlayer* thisclient, CPacket* P)
{
            BEGINPACKET ( pak, 0x7c9 );
            ADDBYTE  (pak, 0);
            ADDQWORD (pak, thisclient->CharInfo->Itemmall);
            thisclient->client->SendPacket( &pak );
            return true;
            ...
            switch (newitem.itemtype) {
                ...
                case 10:
                {
                    ...
                    if (newuse == NULL) {
                        ...
                        storageprice = GETWORD((*P),7) * ((newuse->price / 200 + 1) * (newuse->pricerate + 1));
                    }
                }
                    break;
                case 11:
                {
                    ...
                    if (thisjem == NULL) {
                        ...
                        storageprice = GETWORD((*P),7) * ((thisjem->price / 200 + 1) * (thisjem->pricerate + 1));
                    }
                }
                    break;
                case 12:
                {
                    ...
                    if (newnatural == NULL) {
                        ...
                        storageprice = GETWORD((*P),7) * ((newnatural->price / 200 + 1) * (newnatural->pricerate + 1));
                    }
                }
                    break;
                case 14:
...
{
    if(thisclient->Status->Stance == MOUNTED || thisclient->Status->Stance == DRIVING)
    {
        GServer->SendPM(thisclient,"Cant open a shop with cart/mount");
        return true;
    }
     //LMA 139+:
    ...
    for(int i=0;i<nselling;i++)
    {
        ...
        thisclient->Shop->SellingList[i].price = GETQWORD((*P),n+5+lma_offset);
    ...
    for(int i=0;i<nbuying;i++)
    {
        ...
        thisclient->Shop->BuyingList[slot].price = GETQWORD((*P),n+5+lma_offset);
...
bool CWorldServer::pakShowShop( CPlayer* thisclient, CPacket* P )
{
    ...
    for(unsigned int i = 0; i<otherclient->Shop->Selling;i++)
    {
        ...
        ADDDWORD    ( pak, 0x00000000 );
        ADDWORD     ( pak, 0x0000 );
        ADDQWORD    ( pak, otherclient->Shop->SellingList[i].price );
    }
    for(unsigned int i = 0; i<30;i++)
    {
        if(otherclient->Shop->BuyingList[i].count>0)
        {
            ...
            ADDDWORD    ( pak, 0x00000000 );
            ADDWORD     ( pak, 0x0000 );
            ADDQWORD    ( pak, otherclient->Shop->BuyingList[i].price );
        }
    }
    //ADDBYTE    ( pak, 0x00 );
    thisclient->client->SendPacket( &pak );
...
bool CWorldServer::pakCloseShop( CPlayer* thisclient, CPacket* P )
{
    ...
    for (int i = 1; i <= 14; i++)
    {
        if (thisclient->items[i].itemnum==0&&thisclient->items[i].count<1)
        {
            SetVisibleEquipment(thisclient, i);
            thisclient->SetStats();
        }
    }
    return true;
    ...
    switch(action)
    {
        case 0x01://Gemming
        {
            ...
            if( thisclient->items[srcslot].count<=0 )
            {
                ...
                ADDBYTE    ( pak, 0x12 ); // dont have requer id material
                ADDBYTE    ( pak, 0x00 );
                thisclient->client->SendPacket( &pak );
                return true;
            }

            // Terr0risT - For now all items in FreeROSE can be equipped with a Gem.
            /*
        ...
        case 0x05://Refine
        {
                ...
                if (nb_material_list[0]==0)
                {
                    Log(MSG_WARNING,"%s (refine):: mandatory materials not there (%u=%i).",thisclient->CharInfo->charname,material_list[0],nb_material_list[0]);
                    return true;
                }
                ...
                if(nb_material_list[1]>0){
                    ...
                    powder=true;
                    pc_offset+=NaturalList.Index[thisclient->items[material_list[1]].itemnum]->quality*nb_material_list[1];
                }
                ...
                */
                ...
                if(zulyamount>0)
                {
                    ...
                    //Looks like packet still needed to be sent. - Terr0risT
        ...
        case 0x08:
        {
            ...
            switch (UseList.Index[thisclient->items[material].itemnum]->type)
            {
                case 324:
                {
                    //Normal Separation Drill
                }
                break;

                default:
                {
                    Log(MSG_WARNING,"%s tried to use Separation Drill useitem %u::%u which has an unhandled type %u",thisclient->CharInfo->charname,thisclient->items[material].itemtype,thisclient->items[material].itemnum,UseList.Index[thisclient->items[material].itemnum]->type);
                    return true;
                }
                break;
            }
...
bool CWorldServer::pakRepairItem( CPlayer* thisclient, CPacket* P,int packet_type)
{
    ...
    switch( packet_type)
    {
        case 1:
        {
            ...
            // Terr0risT Fix. Check list_npc and correct data.
            if (testnpc==NULL)
            {
                Log(MSG_HACK,"%s tried to repair with an unknown NPC! NPC id %i in Map %i",thisclient->CharInfo->charname,npc_id,thisclient->Position->Map);
                return true;
            }
...
bool CWorldServer::pakRideRequest( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        ...
        case 0x02://Accept
        {
            ...
            if (thisclient->Status->Stance == MOUNTED)
            {
                GServer->SendPM(thisclient,"Cant accept Cart riding while mounted");
                GServer->SendPM(otherclient,"Cant accept Cart riding while mounted");
                return true;
            }
...
bool CWorldServer::pakItemMallList( CPlayer* thisclient, CPacket* P )
{
	...
	for(int k=0; k < SendTimes; k++)
	{
		...
		if((SendTimes - k) > 1) { ADDBYTE(pak, 47); }
		else { ADDBYTE(pak, count); }
		ADDDWORD(pak, 0x00000000);
		...
		if(CountLeft > 47)
		{
			...
			SendCount = (k*47) + CountLeft;
			CountLeft = 0;
		}

		for(int i = (k*47); i < count; i++)
		{
			MYSQL_ROW row = mysql_fetch_row(result);
			result->current_row++;
            thisclient->ItemMallItemsPrice[place] = atoi(row[3]);
            place++;
			ADDDWORD(pak, i);
...
bool CWorldServer::pakItemMall( CPlayer* thisclient, CPacket* P )
{
     ...
     switch (GETBYTE((*P), 0x0))
     {
            ...
            break;
            case 0x02: // Observe: Gift to Otherplayer
            {
                char name[30];
                memcpy( name, &P->Buffer[4], P->Size );
                if (!GetIfOwnName(thisclient, name))
                {
                if (GetIfCharExists(name))
                {
            ...
            case 0x03:
            {
                 ...
                 //    while(i != 162)
                 //    {
                 //        Log(MSG_INFO, "%i",P->Buffer[i]);
                 //        i++;
                 //    }
                 //    return true;
                 //takes one item to itemmall to player's inventory
                 WORD mal_qty=GETWORD( (*P), 2);
                 BYTE mall_slot=GETBYTE((*P), 4);
            ...
            break;
            case 0x04: // Observe: Are you sure?
            {
                int mileageslot;
                int fromslot = GETBYTE((*P),4);
                char name[30];
                memcpy( name, &P->Buffer[5], P->Size );
                CPlayer* otherclient = GetClientByCharNameCI(name);
                if (otherclient == NULL)
                {
                    mileageslot = GetMileageFreeSlotOffline(name);
                    if (mileageslot != -1)
                    GiftOffline(thisclient, name, fromslot, mileageslot);
                    else
                    {
                        GServer->SendPM(thisclient,"%s Mileage Storage Is Full",name);
                        return true;
                    }
                }
                else
                {
                    mileageslot = GetMileageFreeSlotOnline(otherclient);
                    if (mileageslot != -1)
                    GiftOnline(thisclient, otherclient, fromslot, mileageslot);
                    else
                    {
                        GServer->SendPM(thisclient,"%s Mileage Storage Is Full",name);
                        return true;
...
bool CWorldServer::GiveDasmItems( CPlayer* thisclient,UINT src)
{
   ...
   if(totalprob==0)
   {
    ...
    else
    {
        ...
        nb_items = (rand() % (BreakList[k]->reward_max + 1) + BreakList[k]->reward_min);
        if (nb_items<=0)
        {
            nb_items=1;
        }
    ...
    if(give_all)
    {
        for (int no_reward=0;no_reward<nb_items;no_reward++)
        {
            ...
            mybreak[no_reward].item.durability = 1;
			if(mybreak[no_reward].item.itemtype < 10 || mybreak[no_reward].item.itemtype == 14)
				mybreak[no_reward].item.durability = EquipList[mybreak[no_reward].item.itemtype].Index[mybreak[no_reward].item.itemnum]->durability;
            ...
            if(BreakList[k]->amount_min[no_reward]!=BreakList[k]->amount_max[no_reward])
            {
                ...
                mybreak[no_reward].item.count = (rand() % (BreakList[k]->amount_max[no_reward] + 1) + BreakList[k]->amount_min[no_reward]);
            }
            else
            {
    ...
    else
    {
       ...
       {
           UINT randnum = (rand() % 100000); //RandNumber(0,99999);
           for(int i=0;i<BreakList[k]->nb_reward;i++)
           {
               if(randnum < BreakList[k]->prob[i])
               {
                   m[no_reward] = i;
                   break;
               }
           ...
           if(m[no_reward]<20)
           {
                ...
                if(BreakList[k]->amount_min[m[no_reward]]!=BreakList[k]->amount_max[m[no_reward]])
                {
                    ...
                    mybreak[no_reward].item.count = (rand() % (BreakList[k]->amount_max[m[no_reward]] + 1) + BreakList[k]->amount_min[m[no_reward]]);
                }
                else
                {
...
bool CWorldServer::SeaOfDawn( GAGroup* thisgroup , int map )
{
    for(int i = 0; i<thisgroup->Members.size(); i++)
    {
        ...
        position.x=5200;
        position.y=4772;
        MapList.Index[map]->TeleportPlayer( member, position, false );

        BEGINPACKET( pak, 0x7f8 );
        ADDSTRING    ( pak, "HUD_DawnSea_GameStarted" );
        ADDBYTE    ( pak, 0x00 );
        member->client->SendPacket( &pak );
    }
    GServer->SeaOfDawnSpawn(thisgroup,map);
    return true;
}
bool CWorldServer::SeaOfDawnSpawn( GAGroup* thisgroup , int map )
{
    for(int i=0;i<GServer->ArenaSpawnList.size();i++)
    {
        CCArenaSpawn* thisspawn = GServer->ArenaSpawnList.at(i);
        if(thisspawn->map != map)
            continue;
        if(thisspawn->npc==1)
        {
            MapList.Index[map]->AddNpc(thisspawn->type,thisspawn->position,thisgroup->GroupId);
        }
        else
        {
            MapList.Index[map]->AddAdrenaMonster(thisspawn->type,thisspawn->position,thisgroup->GroupId);
        }
    }
